...
TEST_F(LibAbcKitModifyClassApiTests, ClassAddMethodTest0)
{
    std::string input = ABCKIT_ABC_DIR "ut/extensions/arkts/modify_api/class/class_add_method_static.abc";
    std::string output = ABCKIT_ABC_DIR "ut/extensions/arkts/modify_api/class/class_add_method_static_out.abc";

    AbckitFile *file = nullptr;
    helpers::AssertOpenAbc(input.c_str(), &file);

    helpers::ModuleByNameContext ctxFinder = {nullptr, "class_add_method_static"};
    g_implI->fileEnumerateModules(file, &ctxFinder, helpers::ModuleByNameFinder);

    helpers::MethodByNameContext ctxMethodFinder = {nullptr, "sayHello:f64;void;", true};
    g_implI->moduleEnumerateTopLevelFunctions(ctxFinder.module, &ctxMethodFinder, helpers::MethodByNameFinder);

    helpers::ClassByNameContext ctxClassFinder = {nullptr, "Person"};
    g_implI->moduleEnumerateClasses(ctxFinder.module, &ctxClassFinder, helpers::ClassByNameFinder);

    auto arkKlass = g_implArkI->coreClassToArktsClass(ctxClassFinder.klass);
    auto arkFunc = g_implArkI->coreFunctionToArktsFunction(ctxMethodFinder.method);

    g_implArkM->classAddMethod(arkKlass, arkFunc);
    ASSERT_EQ(g_impl->getLastError(), ABCKIT_STATUS_NO_ERROR);
    g_impl->writeAbc(file, output.c_str(), output.length());
    ASSERT_EQ(g_impl->getLastError(), ABCKIT_STATUS_NO_ERROR);
    g_impl->closeFile(file);
...
TEST_F(LibAbcKitModifyClassApiTests, ClassAddMethodTest1)
{
    std::string input = ABCKIT_ABC_DIR "ut/extensions/arkts/modify_api/class/class_add_method_static_extral.abc";
    std::string first = ABCKIT_ABC_DIR "ut/extensions/arkts/modify_api/class/class_add_method_static_extral_first.abc";
    std::string output = ABCKIT_ABC_DIR "ut/extensions/arkts/modify_api/class/class_add_method_static_extral_out.abc";

    auto outputRst = helpers::ExecuteStaticAbc(input, "class_add_method_static_extral", "main");
    EXPECT_TRUE(helpers::Match(outputRst, "hi\n"));
    // open input file
    AbckitFile *file = nullptr;
    helpers::AssertOpenAbc(input.c_str(), &file);

    helpers::ModuleByNameContext ctxFinder = {nullptr, "class_add_method_static_extral"};
    g_implI->fileEnumerateModules(file, &ctxFinder, helpers::ModuleByNameFinder);

    helpers::MethodByNameContext ctxMethodFinder = {nullptr, "sayHello"};
    g_implI->moduleEnumerateTopLevelFunctions(ctxFinder.module, &ctxMethodFinder, helpers::MethodByNameFinder);
    ...
    ctxFinder = {nullptr, "class_add_method_static_extral"};
    g_implI->fileEnumerateModules(file, &ctxFinder, helpers::ModuleByNameFinder);
    ...
    ctxClassFinder = {nullptr, "Person"};
    g_implI->moduleEnumerateClasses(ctxFinder.module, &ctxClassFinder, helpers::ClassByNameFinder);

    helpers::MethodByNameContext targetCtxFinder = {nullptr, "sayHello"};
    g_implI->classEnumerateMethods(ctxClassFinder.klass, &targetCtxFinder, helpers::MethodByNameFinder);

    ClassAddIfaceTransformMainIr(mainCtxFinder.method, targetCtxFinder.method);
    ...
    outputRst = helpers::ExecuteStaticAbc(output, "class_add_method_static_extral", "main");
    EXPECT_TRUE(helpers::Match(outputRst, "hi\nhello\n"));
}
}  // namespace libabckit::test
