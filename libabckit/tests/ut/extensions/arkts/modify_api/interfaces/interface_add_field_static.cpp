...
static void UpdateMainFunction(const std::string &second, AbckitFile *file, std::string &rst)
{
    helpers::AssertOpenAbc(second.c_str(), &file);
    helpers::ModuleByNameContext ctxFinder = {nullptr, "interface_add_method_static_extral"};
    g_implI->fileEnumerateModules(file, &ctxFinder, helpers::ModuleByNameFinder);

    helpers::MethodByNameContext mainCtxFinder = {nullptr, "main"};
    g_implI->moduleEnumerateTopLevelFunctions(ctxFinder.module, &mainCtxFinder, helpers::MethodByNameFinder);

    helpers::ClassByNameContext classCtxFinder = {nullptr, "NewClass"};
    g_implI->moduleEnumerateClasses(ctxFinder.module, &classCtxFinder, helpers::ClassByNameFinder);

    helpers::MethodByNameContext targetCtxFinder = {nullptr, "sayHello"};
    g_implI->classEnumerateMethods(classCtxFinder.klass, &targetCtxFinder, helpers::MethodByNameFinder);

    ClassAddInterfaceTransformMainIr(mainCtxFinder.method, targetCtxFinder.method);

    g_impl->writeAbc(file, rst.c_str(), rst.length());
    ASSERT_EQ(g_impl->getLastError(), ABCKIT_STATUS_NO_ERROR);
    g_impl->closeFile(file);
    ASSERT_EQ(g_impl->getLastError(), ABCKIT_STATUS_NO_ERROR);
}

TEST_F(LibAbcKitModifyApiIfaceTests, InterfaceAddMethodTest1)
{
    std::string input =
        ABCKIT_ABC_DIR "ut/extensions/arkts/modify_api/interfaces/interface_add_method_static_extral.abc";
    std::string first =
        ABCKIT_ABC_DIR "ut/extensions/arkts/modify_api/interfaces/interface_add_method_static_extral_first.abc";
    std::string second =
        ABCKIT_ABC_DIR "ut/extensions/arkts/modify_api/interfaces/interface_add_method_static_extral_second.abc";
    std::string rst = ABCKIT_ABC_DIR "ut/extensions/arkts/modify_api/interfaces/interface_add_method_static_rst.abc";
    AbckitFile *file = nullptr;
    helpers::AssertOpenAbc(input.c_str(), &file);
    helpers::ModuleByNameContext ctxFinder = {nullptr, "interface_add_method_static_extral"};
    g_implI->fileEnumerateModules(file, &ctxFinder, helpers::ModuleByNameFinder);
    ...
    helpers::InterfaceByNameContext ifaceCtxFinder1 = {nullptr, "MyInterface"};
    g_implI->moduleEnumerateInterfaces(ctxFinder.module, &ifaceCtxFinder1, helpers::InterfaceByNameFinder);
    helpers::MethodByNameContext methodCtxFinder = {nullptr, "sayHello"};
    g_implI->interfaceEnumerateMethods(ifaceCtxFinder.iface, &methodCtxFinder, helpers::MethodByNameFinder);
    auto arkInterface = g_implArkI->coreInterfaceToArktsInterface(ifaceCtxFinder1.iface);
    auto arkFunc = g_implArkI->coreFunctionToArktsFunction(methodCtxFinder.method);
    g_implArkM->interfaceAddMethod(arkInterface, arkFunc);
    g_impl->writeAbc(file, first.c_str(), first.length());
    ASSERT_EQ(g_impl->getLastError(), ABCKIT_STATUS_NO_ERROR);
    g_impl->closeFile(file);
    ASSERT_EQ(g_impl->getLastError(), ABCKIT_STATUS_NO_ERROR);
    helpers::AssertOpenAbc(first.c_str(), &file);
    ctxFinder = {nullptr, "interface_add_method_static_extral"};
    g_implI->fileEnumerateModules(file, &ctxFinder, helpers::ModuleByNameFinder);
    // Get NewClass
    helpers::ClassByNameContext classCtxFinder = {nullptr, "NewClass"};
    g_implI->moduleEnumerateClasses(ctxFinder.module, &classCtxFinder, helpers::ClassByNameFinder);
    // Get TestInterfaceClass
    helpers::ClassByNameContext testClassCtxFinder = {nullptr, "TestInterfaceClass"};
    g_implI->moduleEnumerateClasses(ctxFinder.module, &testClassCtxFinder, helpers::ClassByNameFinder);
    // Get TestInterfaceClass mothed
    helpers::MethodByNameContext funcCtxFinder = {nullptr, "sayHello"};
    g_implI->classEnumerateMethods(testClassCtxFinder.klass, &funcCtxFinder, helpers::MethodByNameFinder);
    g_implArkM->classAddMethod(classCtxFinder.klass->GetArkTSImpl(), funcCtxFinder.method->GetArkTSImpl());
    g_impl->writeAbc(file, second.c_str(), second.length());
    ASSERT_EQ(g_impl->getLastError(), ABCKIT_STATUS_NO_ERROR);
    g_impl->closeFile(file);
    ASSERT_EQ(g_impl->getLastError(), ABCKIT_STATUS_NO_ERROR);

    UpdateMainFunction(second, file, rst);

    auto result = helpers::ExecuteStaticAbc(input, "interface_add_method_static_extral", "main");
    EXPECT_TRUE(helpers::Match(result, "aaa\n"));
    result = helpers::ExecuteStaticAbc(rst, "interface_add_method_static_extral", "main");
    EXPECT_TRUE(helpers::Match(result, "aaa\nbbb\n"));
}
}  // namespace libabckit::test