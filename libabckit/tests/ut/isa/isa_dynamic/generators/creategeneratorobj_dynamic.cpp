...
class LibAbcKitCreateDynCreateGeneratorObjInstTest : public ::testing::Test {};
...
static void TransformIr(AbckitGraph *graph)
{
    ...
    AbckitInst *inst = g_dynG->iCreateCreategeneratorobj(graph, firstInst);
    while (curInst != nullptr) {
        AbckitIsaApiDynamicOpcode curOpcode = g_dynG->iGetOpcode(curInst);
...
// Test: test-kind=api, api=IsaApiDynamicImpl::iCreateCreategeneratorobj, abc-kind=ArkTS1, category=positive,
// extension=c
TEST_F(LibAbcKitCreateDynCreateGeneratorObjInstTest, CreateDynCreategeneratorobjValid)
{
    ...
    helpers::TransformMethod(
        ...
        ABCKIT_ABC_DIR "ut/isa/isa_dynamic/generators/creategeneratorobj_dynamic_modified.abc", "foo",
        ...
        [&](AbckitGraph *graph) {
            std::vector<helpers::BBSchema<AbckitIsaApiDynamicOpcode>> bbSchemas(
                {{{},
                 ...
                 {{0},
                  {2},
                  {{4, ABCKIT_ISA_API_DYNAMIC_OPCODE_CREATEGENERATOROBJ, {3}},
    ...
    output = helpers::ExecuteDynamicAbc(ABCKIT_ABC_DIR
                                        "ut/isa/isa_dynamic/generators/creategeneratorobj_dynamic_modified.abc",
                                        "creategeneratorobj_dynamic");
    EXPECT_TRUE(helpers::Match(output, "\\[object Generator\\]\n"));
}
