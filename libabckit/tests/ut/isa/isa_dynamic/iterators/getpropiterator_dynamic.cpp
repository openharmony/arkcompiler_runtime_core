...
class LibAbcKitCreateDynGetPropIteratorInstTest : public ::testing::Test {};
...
static void TransformIr(AbckitGraph *graph)
{
    ...
    AbckitInst *inst = g_dynG->iCreateGetpropiterator(graph, firstInst);
    while (curInst != nullptr) {
        AbckitIsaApiDynamicOpcode curOpcode = g_dynG->iGetOpcode(curInst);
...
// Test: test-kind=api, api=IsaApiDynamicImpl::iCreateGetpropiterator, abc-kind=ArkTS1, category=positive, extension=c
TEST_F(LibAbcKitCreateDynGetPropIteratorInstTest, CreateDynGetpropiteratorValid)
{
    auto output = helpers::ExecuteDynamicAbc(ABCKIT_ABC_DIR "ut/isa/isa_dynamic/iterators/getpropiterator_dynamic.abc",
                                             "getpropiterator_dynamic");
    EXPECT_TRUE(helpers::Match(output, "10\n"));

    helpers::TransformMethod(
        ABCKIT_ABC_DIR "ut/isa/isa_dynamic/iterators/getpropiterator_dynamic.abc",
        ABCKIT_ABC_DIR "ut/isa/isa_dynamic/iterators/getpropiterator_dynamic_modified.abc", "foo",
        ...
        [&](AbckitGraph *graph) {
            std::vector<helpers::BBSchema<AbckitIsaApiDynamicOpcode>> bbSchemas(
                {{{},
                 ...
                 {{0},
                  {2},
                  {{4, ABCKIT_ISA_API_DYNAMIC_OPCODE_GETPROPITERATOR, {3}},
