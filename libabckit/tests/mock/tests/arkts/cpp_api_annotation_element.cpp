...
class LibAbcKitCppMockArktsTestAnnotationElement : public ::testing::Test {};

// Test: test-kind=mock, api=AnnotationElement::GetName, abc-kind=ArkTS1, category=internal, extension=cpp
TEST_F(LibAbcKitCppMockArktsTestAnnotationElement, AnnotationElement_GetName)
{
    ASSERT_TRUE(CheckMockedStackEmpty());
    {
        abckit::File f(DEFAULT_PATH);
        ASSERT_TRUE(CheckMockedApi("OpenAbc"));
        abckit::mock::helpers::GetMockArktsAnnotationElement(f).GetName();
