...
TEST_F(LSETest, DeletedPhiCandidate)
{
    GRAPH(GetGraph())
    {
        ...
        BASIC_BLOCK(2, 6, 7)
        {
            ...
            INST(4, Opcode::NewArray).ref().Inputs(3, 1).TypeId(273);
            ...
            INST(68, Opcode::NewArray).ref().Inputs(3, 65).TypeId(273);
    ...
    GRAPH(graph_lsed)
    {
        ...
        BASIC_BLOCK(2, 6, 7)
        {
            ...
            INST(4, Opcode::NewArray).ref().Inputs(3, 1).TypeId(273);
            INST(5, Opcode::StoreStatic).ref().Inputs(3, 4).TypeId(3105);
            INST(68, Opcode::NewArray).ref().Inputs(3, 1).TypeId(273);
            INST(69, Opcode::StoreStatic).ref().Inputs(3, 68).TypeId(3105);
...
TEST_F(LSETest, LoopWithOverwrite)
{
    GRAPH(GetGraph())
    {
        ...
        BASIC_BLOCK(4, 3, 4)
        {
            ...
            INST(20, Opcode::LoadObject).ref().Inputs(0).TypeId(242);  // Not eliminable due v33
...
TEST_F(LSETest, EliminatedWithSafePoint)
{
    ...
    GRAPH(graph_lsed)
    {
        ...
        {
            INST(3, Opcode::StoreObject).ref().Inputs(0, 1).TypeId(122);
            INST(5, Opcode::SafePoint).Inputs(0, 1).SrcVregs({0, 1});
...
TEST_F(LSETest, OverInlinedVirtualCall)
{
    GRAPH(GetGraph())
    {
        ...
        BASIC_BLOCK(3, 5)
        {
            ...
            INST(6, Opcode::CallVirtual).v0id().InputsAutoType(1, 2, 20).Inlined();
        ...
        BASIC_BLOCK(4, 5)
        {
            ...
            INST(9, Opcode::CallVirtual).v0id().InputsAutoType(1, 2, 21).Inlined();
...
/**
 * Stores that can be eliminated. But we do not process functions
 * with monitors
 */
TEST_F(LSETest, EliminableMonitoredStores)
{
    std::vector<Graph *> equal_graphs = {GetGraph(), CreateEmptyGraph()};
    for (auto graph : equal_graphs) {
        GRAPH(graph)
        {
            ...
            BASIC_BLOCK(2, -1)
            {
                ...
                INST(2, Opcode::Monitor).v0id().Entry().Inputs(0, 8);
                INST(3, Opcode::StoreObject).s64().Inputs(0, 1).TypeId(243);
                INST(9, Opcode::SaveState).Inputs(0, 1).SrcVregs({0, 1});
                INST(4, Opcode::Monitor).v0id().Exit().Inputs(0, 9);
                INST(5, Opcode::StoreObject).s64().Inputs(0, 1).TypeId(243);
                INST(6, Opcode::StoreObject).s64().Inputs(0, 1).TypeId(243);
...
/**
 * Outer loop overwrites inner loop reference. No elimination
 */
TEST_F(LSETest, OuterOverwrite)
{
    GRAPH(GetGraph())
    {
        ...
        BASIC_BLOCK(6, 5, 6)
        {
            ...
            INST(38, Opcode::LoadObject).ref().Inputs(0).TypeId(194);  // Not eliminable due to below overwrite
...
/**
 * Do not eliminate loads over SafePoints due to GC relocations
 */
...
/**
 * Pool constants can be relocated as well.
 */
...
/**
 * Do not eliminate loads over SafePoints in loops due to GC relocations
 */
TEST_F(LSETest, StoreCandidateWithSafePoint)
{
    GRAPH(GetGraph())
    {
        PARAMETER(0, 0).ref();
        ...
        BASIC_BLOCK(3, 3, 4)
        {
            ...
            INST(8, Opcode::SafePoint).Inputs(0, 7).SrcVregs({0, 1});
