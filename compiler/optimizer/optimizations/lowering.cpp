...
void Lowering::VisitCastValueToAnyType([[maybe_unused]] GraphVisitor *v, Inst *inst)
{
    ...
    if (AnyBaseTypeToDataType(any_type) == DataType::REFERENCE) {
        return;
    }
    // from
...
void Lowering::LowerShift(Inst *inst)
{
    ...
    Inst *new_inst;
    if (opc == Opcode::Shr) {
        new_inst = graph->CreateInstShrI(inst->GetType(), inst->GetPc(), val);
    } else if (opc == Opcode::AShr) {
        new_inst = graph->CreateInstAShrI(inst->GetType(), inst->GetPc(), val);
    } else {
        new_inst = graph->CreateInstShlI(inst->GetType(), inst->GetPc(), val);
    }
...
Inst *Lowering::LowerAddSub(Inst *inst)
{
    ...
    Inst *new_inst;
    if (is_add) {
        new_inst = graph->CreateInstAddI(inst->GetType(), inst->GetPc(), static_cast<uint64_t>(val));
    } else {
        new_inst = graph->CreateInstSubI(inst->GetType(), inst->GetPc(), static_cast<uint64_t>(val));
    }
    new_inst->SetInput(0, inst->GetInput(0).GetInst());
    InsertInstruction(inst, new_inst);
    return new_inst;
...
void Lowering::LowerMulDivMod(Inst *inst)
{
    ...
    if constexpr (opcode == Opcode::Mul) {
        new_inst = graph->CreateInstMulI(inst->GetType(), inst->GetPc(), static_cast<uint64_t>(val));
        ...
        new_inst = graph->CreateInstDivI(inst->GetType(), inst->GetPc(), static_cast<uint64_t>(val));
        if (graph->IsBytecodeOptimizer()) {
        ...
        new_inst = graph->CreateInstModI(inst->GetType(), inst->GetPc(), static_cast<uint64_t>(val));
        if (graph->IsBytecodeOptimizer()) {
            inst->ClearFlag(compiler::inst_flags::NO_DCE);  // In Bytecode Optimizer Div may have NO_DCE flag
            if (val == 0) {
                new_inst->SetFlag(compiler::inst_flags::NO_DCE);
...
Inst *Lowering::LowerMultiplyAddSub(Inst *inst)
{
    ...
    Inst *new_inst = is_sub ? graph->CreateInstMSub(inst->GetType(), inst->GetPc())
                            : graph->CreateInstMAdd(inst->GetType(), inst->GetPc());
...
Inst *Lowering::LowerNegateMultiply(Inst *inst)
{
    ...
    Inst *new_inst = graph->CreateInstMNeg(inst->GetType(), inst->GetPc());
    SetInputsAndInsertInstruction(operands, inst, new_inst);
    return new_inst;
}
...
void Lowering::LowerLogicWithInvertedOperand(Inst *inst)
{
    ...
    if (opcode == Opcode::Or) {
        ...
        new_inst = graph->CreateInstOrNot(inst->GetType(), inst->GetPc());
    } else if (opcode == Opcode::And) {
        if (!encoder->CanEncodeAndNot()) {
            return;
        }
        new_inst = graph->CreateInstAndNot(inst->GetType(), inst->GetPc());
    } else {
        if (!encoder->CanEncodeXorNot()) {
            return;
        }
        new_inst = graph->CreateInstXorNot(inst->GetType(), inst->GetPc());
    }
...
Inst *Lowering::LowerBinaryOperationWithShiftedOperand(Inst *inst)
{
    ...
    using InvertedOperandMatcher = AnyOf<BinaryOp<OPCODE, SRC0, NOT<SHLI<SRC1>>>,
...
Inst *Lowering::LowerLogic(Inst *inst)
{
    ...
    Inst *new_inst;
    if (opc == Opcode::Or) {
        new_inst = graph->CreateInstOrI(inst->GetType(), inst->GetPc(), val);
    } else if (opc == Opcode::And) {
        new_inst = graph->CreateInstAndI(inst->GetType(), inst->GetPc(), val);
    } else {
        new_inst = graph->CreateInstXorI(inst->GetType(), inst->GetPc(), val);
    }
...
void Lowering::LowerConstArrayIndex(Inst *inst, Opcode low_level_opcode)
{
    ...
    if (input_inst->IsConst() || input_inst->GetOpcode() == Opcode::BoundsCheckI) {
        uint64_t value;
        if (input_inst->IsConst()) {
            value = input_inst->CastToConstant()->GetIntValue();
        } else {
            ASSERT(low_level_opcode != Opcode::BoundsCheckI);
            value = input_inst->CastToBoundsCheckI()->GetImm();
        }
...
void Lowering::LowerStateInst(SaveStateInst *save_state)
{
    ...
    while (idx < inputs_count) {
        ...
        if (input_inst->GetOpcode() == Opcode::NullPtr ||
            ...
            save_state->AppendImmediate(raw_value, vreg.Value(), input_inst->GetType(), vreg.IsAccumulator());
...
void Lowering::LowerToDeoptimizeCompare(Inst *inst)
{
    ...
    if ((cmpInp1->IsConst() && ConstantFitsCompareImm(cmpInp1, size, compare->GetCc())) || cmpInp1->IsNullPtr()) {
        uint64_t imm = cmpInp1->IsNullPtr() ? 0 : cmpInp1->CastToConstant()->GetIntValue();
