 ...
 */
class LineString : public BaseString {  // NOLINT(cppcoreguidelines-special-member-functions)
public:
    BASE_CAST_CHECK(LineString, IsLineString);
    NO_MOVE_SEMANTIC_CC(LineString);
    NO_COPY_SEMANTIC_CC(LineString);

    static constexpr uint32_t MAX_LENGTH = 0x0FFFFFF0U;  // Maximum length of the string, (1 << 28) -16
