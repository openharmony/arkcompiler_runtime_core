...
The test suite SHALL provide a reusable fake HDC object in `test/mock/hdc.py` that records every `run` and `shell` invocation and returns configured `HdcResult` values without executing external commands.
...
The test suite SHALL provide a reusable fake device object in `test/mock/device.py` with configurable screen bounds, process state, invalid bundles, remote directories, remote files, and failure flags for device operations used by benchmark execution.
