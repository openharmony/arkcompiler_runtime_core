...
The test suite SHALL provide a reusable fake device object in `test/mock/device.py` with configurable screen bounds, process state, invalid bundles, remote directories, remote files, health observations, and failure flags for device operations used by benchmark execution, including reboot, boot readiness, wakeup, screen-timeout disabling, hilog operations, app termination, and screenshot capture.
