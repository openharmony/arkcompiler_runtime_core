# ui-input-recording Specification

## Purpose

Defines recording device UI inputs with `record.py` and converting them into editable benchmark flow JSON.
...
#### Scenario: Unexpected recorder exit fails
- **WHEN** the recorder process exits after readiness before the system sends SIGINT for a manual or timeout stop
- **THEN** the system reports the recorder return code and stderr, does not read `/data/local/tmp/record.csv`, and does not write a flow file
