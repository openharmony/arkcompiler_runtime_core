...
Documentation of meta information generated by the compiler is located in `code_metainfo.md`.
...
The compiler is aimed to translate managed function bytecode to native code. The current runtime/compiler interface uses
`ark::CompilerInterface::CompileMethod(...)` to request JIT or OSR compilation. When compilation succeeds, runtime
installs the generated entrypoint so later calls can execute native code.
...
Compiled code does not rely on arbitrary `ManagedThread` fields directly. It uses a small ABI-visible subset whose
offsets are exposed from `managed_thread.h`:

| Field                | Type                  | Description |
| ---                  | ----                  | ----------- |
| frame_               | `ark::Frame*` / top frame pointer | Current managed frame pointer. |
| isCompiledFrame_     | `bool`                | Whether the current top frame is compiled. |
| exception_           | `ark::ObjectHeader*`  | Pending exception object, or `nullptr`. |
| entrypointsTable_    | `void*[]`             | Runtime entrypoints table (See [Runtime entrypoints](#runtime-entrypoints)). |
| nativePc_            | `uintptr_t`           | Native PC used by stack walking for compiled frames. |
...
A table of the entrypoints is located in `ark::ManagedThread::entrypointsTable_` and can be accessed via the thread
register. The compiler-visible surface is generated from `runtime/entrypoints/entrypoints.yaml`.
...
### Hotness counter
The field `hotness_counter_` reflects the remaining budget before profiling or compilation work should start. The
interpreter decrements it on calls and backward edges, and when the counter reaches zero or below runtime may start
profiling, JIT compilation, or OSR depending on thresholds and context.
...
The exact compiled-frame and bridge-frame layout is defined by `CFrameLayout` and `CFrame`. Treat the diagram below as
conceptual background, not as the ABI source of truth.
...
* Mark the top frame as compiled by updating `ark::ManagedThread::isCompiledFrame_`.
...
* Mark the top frame as interpreter-managed again by restoring `ark::ManagedThread::isCompiledFrame_`.
...
* Mark the top frame as interpreter-managed by updating `ark::ManagedThread::isCompiledFrame_`.
...
* Drop the boundary stack frame.  
* Restore `ark::ManagedThread::isCompiledFrame_` back to compiled-frame state.
...
Stack unwinding always starts from the top-frame state stored in `ark::ManagedThread`: `frame_`,
`isCompiledFrame_`, and `nativePc_`. If the current top frame is interpreted, `frame_` points to the interpreter
frame. If the current top frame is compiled, `frame_` and `nativePc_` give `StackWalker` enough state to recover the
current compiled frame and its stack-map metadata.

In practice, stack traversal is implemented by `StackWalker`, which understands interpreter frames, compiled CFrames,
boundary frames, and inlined frames. Having a pointer to the top stack frame, its kind and structure the runtime can
move to the next frame. Moving to the next frame is done according to the table below:
...
during handling a compiled code stack frame it uses meta information generated by the compiler (See `code_metainfo.md`).
...
To do that the runtime uses information about callee-saved registers stored on the stack. This information is generated by the compiler (See `code_metainfo.md`).
...
The runtime determines kind of the top stack frame by reading `ark::ManagedThread::isCompiledFrame_` together with the
...
* Stores the pointer to the exception object to `ark::ManagedThread::exception_`
...
Finding a catch handler in a compiled code stack frame is performed according meta information generated by the compiler (See `code_metainfo.md`).

The interpreter must ignore the returned value if `ark::ManagedThread::exception_` is not 0.
