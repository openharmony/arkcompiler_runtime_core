...
struct IsLazyStreamValue {
    ...
    static constexpr bool VALUE = sizeof(F<Value>(nullptr)) == 1 && sizeof(G<Value>(nullptr)) == 1;
};
...
C ContainerOf(S stream,
              typename std::decay<
                  decltype((*static_cast<C *>(nullptr))
...
C ContainerOf(S stream,
              typename std::decay<
                  decltype((*static_cast<C *>(nullptr))
                               .insert(*static_cast<std::decay_t<decltype(*((*(static_cast<S *>(nullptr)))()))> *>(
                                   nullptr)))>::type *tmp = nullptr)
{
    (void)tmp;
    C c;
