...
module TestRunner
  class SingleTestRunner
    ...
    def run_verifier
      run_command "#{$verifier} #{@verifier_options} #{@verifier_config_args} #{@bin_file}"
    end
    ...
    def run_panda
      ...
      run_command "#{$panda} #{$panda_options.join ' '} #{@test_panda_options} #{aot} " \
    ...
    def run_paoc
      run_command "#{$paoc} --paoc-panda-files #{@bin_file} --paoc-output #{@bin_file}.aot"
    end
