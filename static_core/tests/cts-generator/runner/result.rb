...
module TestRunner
  ...
  class Result
    ...
    def self.write_report
      ...
      TestRunner.log 2, 'Passed:'
      TestRunner.log 2, @@stats[:passed][:files].sort.to_yaml.to_s
      TestRunner.log 1, 'Failed:'
      TestRunner.log 1, @@stats[:failed][:files].sort.to_yaml.to_s
      TestRunner.log 1, 'Failed (coredump):'
      TestRunner.log 1, @@stats[:core][:files].sort.to_yaml.to_s
      TestRunner.log 1, 'Compilation error:'
      TestRunner.log 1, @@stats[:compilation_error][:files].sort.to_yaml.to_s
      TestRunner.log 1, 'Quickening error:'
      TestRunner.log 1, @@stats[:quickening_error][:files].sort.to_yaml.to_s
      TestRunner.log 2, '----------------------------------------'
      ...
      TestRunner.log 1, "Quickening error: #{@@stats[:quickening_error][:cnt]}"
    ...
    def self.failed?
      (
        @@stats[:failed][:cnt] +
        @@stats[:compilation_error][:cnt] +
        @@stats[:quickening_error][:cnt]
      ).positive?
    end
  end
end
