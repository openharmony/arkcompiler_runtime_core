...
class SpecReport:
    """
    Generates an ArkTS specification coverage report in markdown and YAML formats.
    Maps test results to spec sections parsed from a PDF file and calculates
    pass/fail/ignored counters per section.
    """
    ...
    _PH_DATE = "${Date}"
    _PH_TEST_SUITE = "${TestSuite}"
    _PH_RESULT = "${Result}"
    _PH_SPEC_FILE = "${SpecFile}"
    _PH_SPEC_CREATION_DATE = "${SpecCreationDate}"
    _PH_EXCLUDED_HEADER = "${ExcludedHeader}"
    _PH_EXCLUDED_DIVIDER = "${ExcludedDivider}"
    _EXCLUDED_HEADER = " Excluded after |"
    _EXCLUDED_DIVIDER = "-------|"
    ...
    @staticmethod
    def obj_to_dict(dumper: yaml.SafeDumper, data: SpecNode) -> yaml.nodes.MappingNode:
    ...
    def __fmt_str(src: str) -> str:
        ...
        return src.translate(escape_map)
    ...
    def populate_report_md(self) -> None:
        ...
        with open(template_path, encoding="utf-8") as file:
            report = file.read()
        report = report.replace(self._PH_DATE, str(date.today()))
        report = report.replace(self._PH_TEST_SUITE, self.test_suite)
        report = report.replace(self._PH_SPEC_FILE, self.__fmt_str(str(self.spec_file)))
        report = report.replace(self._PH_SPEC_CREATION_DATE, self.spec_creation_date)
        excluded_header = self._EXCLUDED_HEADER if self.has_excluded else ""
        excluded_divider = self._EXCLUDED_DIVIDER if self.has_excluded else ""
        report = report.replace(self._PH_EXCLUDED_HEADER, excluded_header)
        report = report.replace(self._PH_EXCLUDED_DIVIDER, excluded_divider)

        lines: list[str] = self.__report_nodes(self.spec.children)
        lines.append(self.__report_node(self.spec))  # root node - summary line at the bottom
        result = "\n".join(lines)
        report = report.replace(self._PH_RESULT, result)
        _LOGGER.default(f"Spec coverage report is saved to '{self.report_file_md}'")
    ...
    def populate_report_yaml(self) -> None:
        ...
        with open(self.report_file_yaml, mode="w", encoding="utf-8") as file:
            yaml.safe_dump(res, file, default_flow_style=False, sort_keys=False)

    def __calculate_one_test(self, test: Test) -> None:
        """Walks the spec tree along the test's directory path and updates counters at each level."""
        node = self.spec
    ...
    def __update_counters(self, test: Test, node: SpecNode) -> None:
        """Updates accumulated (recursive) counters on the node based on test outcome."""
        node.total_acc += 1
        if test.excluded:
    ...
    def __update_last_node_counters(self, test: Test, node: SpecNode) -> None:
        """Updates own (non-accumulated) counters on the deepest matched node."""
        node.total += 1
        if test.excluded:
    ...
    def __report_node(self, node: SpecNode) -> str:
        """Formats a single SpecNode as a markdown table row."""
    ...
    def __report_nodes(self, nodes: list[SpecNode]) -> list[str]:
        lines = []
        for node in nodes:
            lines.append(self.__report_node(node))
            lines.extend(self.__report_nodes(node.children))
        return lines
