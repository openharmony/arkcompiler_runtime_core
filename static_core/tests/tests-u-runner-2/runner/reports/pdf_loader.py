...
class PdfLoader:
    """
    Reads an ArkTS specification PDF file and builds a tree of SpecNode objects
    from its table of contents (PDF outline / bookmarks).
    """

    def __init__(self, fpath: Path, config: dict | None):
    ...
    @staticmethod
    def __is_destination(field: list | Destination) -> bool:
        """Checks whether the given outline item is a Destination (leaf) rather than a nested list."""
        return str(type(field)).find("Destination") > 0

    def parse(self) -> PdfLoader:
        """Reads the PDF file, extracts metadata and outline, and builds the SpecNode tree."""
        with open(self.spec_file, "rb") as stream:
            reader: PdfReader = PdfReader(stream)
            metadata: DocumentInformation | None = reader.metadata
    ...
    def __read_dest(self, item: Destination, counter: int, parent: SpecNode) -> SpecNode:
        """Creates a SpecNode from a single PDF outline Destination entry and attaches it to the parent."""
