...
| `ets-cts-stackless` | CTS tests running with the stackless coroutine model |
...
|---|---|
| `ets-cts`, `ets-cts-6.0`, `ets-cts-7.0`, `ets-cts-stackless`, `ets-func-tests`, `ets-runtime`, `ets-sdk`, `ets-gc-stress`, `unit-tests` | `ark`, `ark-aot`, `ark-aot-pgo`, `es2panda-verifier`, `hybrid-ark`, `hybrid-es2panda-verifier`, `hybrid-panda-int`, `hybrid-panda-jit`, `panda-all`, `panda-aot`, `panda-aot-pgo`, `panda-int`, `panda-jit`, `panda-jit-repeats` |
...
```
<suite-name>[-<info>]-<kind>[-<OS>][-<arch>][-<config>][-<interpreter>][-<sanitizer>][-OL<opt-level>][-DI][-FULLASTV][-SIMULTANEOUS][-STACKLESS][-REPEATS][-<gc-type>][-<build-type>].txt
```
...
| `STACKLESS` | literal `STACKLESS`                                                  | Included only when `--stackless-coros=true` es2panda arg is set                                                        |
