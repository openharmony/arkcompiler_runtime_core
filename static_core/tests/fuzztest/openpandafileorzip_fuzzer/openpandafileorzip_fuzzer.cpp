...
void OpenPandaFileOrZipFuzzTest(const uint8_t *data, size_t size)
{
    ...
    int ret1 = ark::CreateOrAddFileIntoZip(uncompress_zip_filename, filename1, data, size, APPEND_STATUS_CREATE,
                                           Z_NO_COMPRESSION);
    int ret2 = ark::CreateOrAddFileIntoZip(uncompress_zip_filename, filename2, data, size, APPEND_STATUS_ADDINZIP,
                                           Z_NO_COMPRESSION);
    if (ret1 != 0 || ret2 != 0) {
    ...
    ret1 = ark::CreateOrAddFileIntoZip(uncompress_zip_filename, filename1, data, size, APPEND_STATUS_CREATE,
                                       Z_BEST_COMPRESSION);
    ret2 = ark::CreateOrAddFileIntoZip(uncompress_zip_filename, filename2, data, size, APPEND_STATUS_ADDINZIP,
                                       Z_BEST_COMPRESSION);
    if (ret1 != 0 || ret2 != 0) {
    ...
    {
        ark::panda_file::OpenPandaFileOrZip(uncompress_zip_filename);
        ark::panda_file::OpenPandaFileOrZip(compressed_zip_filename);
    }
    (void)remove(uncompress_zip_filename);
    (void)remove(compressed_zip_filename);
}
