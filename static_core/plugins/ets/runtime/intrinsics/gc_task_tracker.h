...
public:
    ...
    void SetCallbackForTask(uint64_t taskId, mem::Reference *callbackRef);
...
private:
    ...
    uint64_t currentTaskId_ GUARDED_BY(lock_) = 0;
    std::unordered_map<uint64_t, mem::Reference *> callbackRefs_ GUARDED_BY(lock_);
};
