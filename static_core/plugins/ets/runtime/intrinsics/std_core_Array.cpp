...
EtsObject *EtsStdCoreArrayGet(EtsStdCoreArray *array, int32_t index)
{
    return array->StdCoreArrayGet(index);
}

void EtsStdCoreArraySet(EtsStdCoreArray *array, int32_t index, EtsObject *value)
{
    return array->StdCoreArraySet(index, value);
}
...
static constexpr size_t MaxChars()
{
    ...
    if constexpr (std::is_integral_v<T>) {
        ...
        auto sigDigits = static_cast<size_t>(std::ceil(bits * digitPerBit)) + 1U;  // 17
...
static ark::ets::EtsString *EtsStdCoreArrayJoinUtf8String(EtsHandle<EtsObjectArray> &bufferHandle, size_t actualLength,
...
static ark::ets::EtsString *EtsStdCoreArrayJoinUtf8(ElementComputeResult &res, EtsHandle<EtsObjectArray> &buffer,
                                                    size_t actualLength, EtsHandle<EtsString> &separatorHandle)
{
    ...
    if (res.stringCount == actualLength) {
        return EtsStdCoreArrayJoinUtf8String(buffer, actualLength, res.utf8Size, separatorHandle);
    }
...
// Might trigger GC
static ark::ets::EtsString *EtsStdCoreArrayJoinUtf16(ElementComputeResult &res, EtsHandle<EtsObjectArray> &bufferHandle,
                                                     EtsInt actualLength, EtsHandle<EtsString> &separatorHandle)
{
...
extern "C" ark::ets::EtsString *EtsStdCoreArrayJoinInternal(ObjectHeader *buffer, EtsInt actualLength,
                                                            ark::ets::EtsString *separator)
{
    ...
    if (res.utf16Size > 0) {
        return EtsStdCoreArrayJoinUtf16(res, bufferHandle, actualLength, separatorHandle);
    }

    return EtsStdCoreArrayJoinUtf8(res, bufferHandle, actualLength, separatorHandle);
}

extern "C" EtsObject *EtsStdCoreArrayGetUnsafe(EtsStdCoreArray *array, int32_t index)
{
    return array->StdCoreArrayGetUnsafe(index);
}

extern "C" void EtsStdCoreArraySetUnsafe(EtsStdCoreArray *array, int32_t index, EtsObject *value)
{
    array->StdCoreArraySetUnsafe(index, value);
}
