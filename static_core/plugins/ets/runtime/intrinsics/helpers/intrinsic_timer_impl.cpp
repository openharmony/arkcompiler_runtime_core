...
ManagedEntrypoint::ManagedEntrypoint(ManagedEntrypoint &&epInfo)
    : entrypoint_(std::exchange(epInfo.entrypoint_, nullptr))
{
}

ManagedEntrypoint &ManagedEntrypoint::operator=(ManagedEntrypoint &&epInfo)
{
    if (this == &epInfo) {
        return *this;
    }
    entrypoint_ = std::exchange(epInfo.entrypoint_, entrypoint_);
    return *this;
}

ManagedEntrypoint::~ManagedEntrypoint()
{
    PandaVM::GetCurrent()->GetGlobalObjectStorage()->Remove(entrypoint_);
}

void ManagedEntrypoint::Invoke() const
{
    auto *mThread = ManagedThread::GetCurrent();
    ScopedManagedCodeThread managedScope(mThread);
    auto *lambda = mThread->GetVM()->GetGlobalObjectStorage()->Get(entrypoint_);
    LambdaUtils::InvokeVoid(mThread, EtsObject::FromCoreType(lambda));
}

void NativeEntrypoint::Invoke() const
{
    entrypoint_(param_);
}
...
public:
    TimerInfo(EntrypointInfo &&entrypoint, int delay, bool isPeriodic, TimerId id)
        : entrypoint_(std::move(entrypoint)), delay_(delay), isPeriodic_(isPeriodic), id_(id)
    {
    }
    ...
    const EntrypointInfo &GetEntrypoint() const
    {
        return entrypoint_;
    }
...
private:
    const EntrypointInfo entrypoint_;
    const int delay_;
...
static void InvokeEntrypoint(const EntrypointInfo &entrypoint)
{
    if (std::holds_alternative<ManagedEntrypoint>(entrypoint)) {
        std::get<ManagedEntrypoint>(entrypoint).Invoke();
        return;
    }
    ASSERT(std::holds_alternative<NativeEntrypoint>(entrypoint));
    std::get<NativeEntrypoint>(entrypoint).Invoke();
}

static bool CheckJobSwitchEnabled(JobExecutionContext *executionCtx)
{
...
struct EntrypointParams {
    ...
    EntrypointParams(JobManager *jobMan, EventId eId, EntrypointInfo &&epInfo, int delay, bool isPeriodic, TimerId tId)
        : event(jobMan, eId), timer(std::move(epInfo), delay, isPeriodic, tId)
    {
    }

    // CC-OFFNXT(G.FUN.01-CPP) solid logic
    static EntrypointParams *Create(JobManager *jobMan, EventId eId, EntrypointInfo &&epInfo, int delay,
                                    bool isPeriodic, TimerId tId)
    {
        auto allocator = Runtime::GetCurrent()->GetInternalAllocator();
        return allocator->New<EntrypointParams>(jobMan, eId, std::move(epInfo), delay, isPeriodic, tId);
    }
...
void LaunchTimerCallback(EntrypointParams *epParams, TimerEvent *timerEvent)
{
    ...
    auto jobName = "Timer callback: " + ark::ToPandaString(epParams->timer.GetId());
...
void TimerEntrypoint(void *param)
{
    ...
    InvokeEntrypoint(epParams->timer.GetEntrypoint());
    ...
    LaunchTimerCallback(epParams, timerEvent);
}

TimerId CreateTimer(EntrypointInfo timerEntrypoint, int32_t delay, bool periodic)
{
    auto *executionCtx = JobExecutionContext::GetCurrent();
    if (!CheckJobSwitchEnabled(executionCtx)) {
        return 0;
    }
    ...
    auto *jobMan = executionCtx->GetManager();
    auto *epParams = EntrypointParams::Create(jobMan, eventId, std::move(timerEntrypoint), delay, periodic, timerId);
    ASSERT(epParams != nullptr);

    LaunchTimerCallback(epParams, &epParams->event);
    return timerId;
}
