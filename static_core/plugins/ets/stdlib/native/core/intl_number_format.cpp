...
#include "intl_state.h"
#include "intl_number_formatters.h"
#include "intl_number_format.h"
...
static ani_status CheckBindStatus(ani_status status)
{
    return (status == ANI_OK || status == ANI_ALREADY_BINDED) ? ANI_OK : status;
}

static ani_status BindNumberFormatNativeMethods(ani_env *env, ani_class numberFormatClass)
{
    ...
    return CheckBindStatus(env->Class_BindNativeMethods(numberFormatClass, methods.data(), methods.size()));
}

static ani_status BindNumberFormatStaticMethods(ani_env *env, ani_class numberFormatClass)
{
    ...
    return CheckBindStatus(
        env->Class_BindStaticNativeMethods(numberFormatClass, staticMethods.data(), staticMethods.size()));
}

static void InitResolvedNumberFormatOptionRefs(ani_env *env)
{
    const auto resolvedOptionFields = std::array {
        std::pair {"_locale", &refs::g_locale_field},
        std::pair {"_compactDisplay", &refs::g_compactDisplay_field},
        std::pair {"_currencySign", &refs::g_currencySign_field},
        std::pair {"_currency", &refs::g_currency_field},
        std::pair {"_currencyDisplay", &refs::g_currencyDisplay_field},
        std::pair {"minFracStr", &refs::g_minFractionDigits_field},
        std::pair {"maxFracStr", &refs::g_maxFractionDigits_field},
        std::pair {"minSignStr", &refs::g_minSignificantDigits_field},
        std::pair {"maxSignStr", &refs::g_maxSignificantDigits_field},
        std::pair {"minIntStr", &refs::g_minIntegerDigits_field},
        std::pair {"_notation", &refs::g_notation_field},
        std::pair {"_numberingSystem", &refs::g_numberingSystem_field},
        std::pair {"_signDisplay", &refs::g_signDisplay_field},
        std::pair {"_style", &refs::g_style_field},
        std::pair {"_unit", &refs::g_unit_field},
        std::pair {"_unitDisplay", &refs::g_unitDisplay_field},
        std::pair {"useGroupingStr", &refs::g_useGrouping_field},
    };

    for (const auto &[name, field] : resolvedOptionFields) {
        ANI_FATAL_IF_ERROR(env->Class_FindField(refs::g_resolvedOptionsClass, name, field));
    }
}

static void InitNumberFormatRefs(ani_env *env, ani_class numberFormatClass)
{
    ...
    InitResolvedNumberFormatOptionRefs(env);
}
...
ani_status RegisterIntlNumberFormatNativeMethods(ani_env *env)
{
    ...
    ani_status status = BindNumberFormatNativeMethods(env, numberFormatClass);
    if (status != ANI_OK) {
        return status;
    }

    status = BindNumberFormatStaticMethods(env, numberFormatClass);
    if (status != ANI_OK) {
        return status;
    }

    InitNumberFormatRefs(env, numberFormatClass);
    return ANI_OK;
}
