...
public:
    ...
    {
        return *reinterpret_cast<uintptr_t *>(&pool->current_pos_);
    }

    static uintptr_t &GetFreeList(const std::unique_ptr<Pool> &pool)
    {
        return *reinterpret_cast<uintptr_t *>(&pool->free_list_);
    }
...
TEST_F(ItemsPoolGTest, test_1)
{
    ...
    uintptr_t &free_list = GetFreeList(pool);
    ASSERT_EQ(free_list, 0);

    auto &current_pos = GetCurrentPos(pool);
    ASSERT_EQ(current_pos, data);
}
...
// |         |       |       |       |       |       |       |       |       |       |       |       |       |       |       |   0   |       |       |       |       |
    ...
    ASSERT_EQ(free_list, 0);
    ASSERT_EQ(current_pos, data + 1 * PADDED_ITEM_SIZE);
    ...
    ASSERT_EQ(free_list, 0);
    ASSERT_EQ(current_pos, data + 2 * PADDED_ITEM_SIZE);
    ...
    ASSERT_EQ(free_list, 0);
    ASSERT_EQ(current_pos, data + 3 * PADDED_ITEM_SIZE);

    // Step4, Free
    pool->FreeItem(item1);
    ASSERT_EQ(free_list, data + 1 * PADDED_ITEM_SIZE);
    ASSERT_EQ(current_pos, data + 3 * PADDED_ITEM_SIZE);
    ...
    ASSERT_EQ(free_list, 0);
    ASSERT_EQ(current_pos, data + 4 * PADDED_ITEM_SIZE);
    ...
    ASSERT_EQ(free_list, 0);
    ASSERT_EQ(current_pos, data + 5 * PADDED_ITEM_SIZE);
    ...
    ASSERT_EQ(free_list, 0);
    ASSERT_EQ(current_pos, data + 6 * PADDED_ITEM_SIZE);
    ...
    ASSERT_EQ(free_list, 0);
    ASSERT_EQ(current_pos, data + 7 * PADDED_ITEM_SIZE);
    ...
    ASSERT_EQ(free_list, 0);
    ASSERT_EQ(current_pos, data + 8 * PADDED_ITEM_SIZE);
    ...
    ASSERT_EQ(pool->GetIndexByItem(item0), 0b000);
    pool->FreeItem(item0);
    ASSERT_EQ(free_list, data + 0 * PADDED_ITEM_SIZE);
    ASSERT_EQ(current_pos, data + 8 * PADDED_ITEM_SIZE);

    // Step14, Free
    ASSERT_EQ(pool->GetIndexByItem(item5), 0b100);
    pool->FreeItem(item5);
    ASSERT_EQ(free_list, data + 4 * PADDED_ITEM_SIZE);
    ASSERT_EQ(current_pos, data + 8 * PADDED_ITEM_SIZE);

    // Step15, Free
    ASSERT_EQ(pool->GetIndexByItem(item4), 0b011);
    pool->FreeItem(item4);
    ASSERT_EQ(free_list, data + 3 * PADDED_ITEM_SIZE);
    ASSERT_EQ(current_pos, data + 8 * PADDED_ITEM_SIZE);
...
// |         |       |   0   |       |       |
    ...
    ASSERT_NE(data, 0);
    ASSERT_EQ(current_pos, data + 0 * PADDED_ITEM_SIZE);
    ASSERT_EQ(free_list, 0);
...
TEST_F(ItemsPoolGTest, test_IsValidItem)
{
    auto pool = ItemsPoolTest::CreatePool();
    uintptr_t data = GetData(pool);
    uintptr_t data_end = data + pool->MAX_POOL_SIZE;
    ...
    ASSERT_EQ(pool->IsValidItem(reinterpret_cast<decltype(item)>(data_end)), false);
    ASSERT_EQ(pool->IsValidItem(reinterpret_cast<decltype(item)>(data - sizeof(*item))), false);
    ASSERT_EQ(pool->IsValidItem(reinterpret_cast<decltype(item)>(data_end - sizeof(*item))), true);
}
