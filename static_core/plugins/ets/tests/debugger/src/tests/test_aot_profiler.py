...
from arkdb.compiler import AotCompiler, AotOptions, StringCodeCompiler, ark_aot_compile
...
async def test_aot_profiler(
    log: RichLogger,
    code_compiler: StringCodeCompiler,
    ark_aot_compiler: AotCompiler,
    ark_runtime: Runtime,
    nursery: trio.Nursery,
    debug_locator: DebugLocator,
) -> None:
    script_file = code_compiler.compile(CODE)
    o = ark_runtime.options
    boot_panda_files = [str(f) for f in o.boot_panda_files + script_file.boot_abc]
    aot_file = str(script_file.panda_file).replace(".abc", ".an")
    stdout, stderr = ark_aot_compiler.aot_compile(
        boot_panda_files=boot_panda_files,
        entry_abc=script_file.entry_abc,
        output_file=aot_file,
    )
    if stderr:
        log.warning("AOT compiler stderr: %s", stderr)
    async with ark_runtime.run(
        nursery, module=script_file, profile=True, aot=True, aot_file=aot_file
    ) as process:
