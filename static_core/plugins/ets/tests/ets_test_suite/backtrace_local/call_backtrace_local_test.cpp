...
TEST_F(BackTraceLocalTest, BackTraceLocalTestCase)
{
    ...
    if (pandafile == nullptr) {
        LOG(FATAL, TOOLING)
            << "ARK_ETS_GTEST_ABC_PATH env variable should be set and point to call_backtrace_local.abc";
    }
    options.SetBootPandaFiles({stdlib});
    options.SetPandaFiles({pandafile});
    bool success = Runtime::Create(options);
    ASSERT_TRUE(success) << "Cannot create Runtime";
    auto result = Runtime::GetCurrent()->ExecutePandaFile(pandafile, "call_backtrace_local.ETSGLOBAL::main", {});
    EXPECT_TRUE(result);
}
}  // namespace ark::test
