...
#include "aniclassfindstaticmethod_fuzzer.h"
...
void AniClassFindStaticMethodFuzzTest(const char *data, size_t size)
{
    if (size <= 0U) {
        return;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::AniClassFindStaticMethodFuzzTest(reinterpret_cast<const char *>(data), size);
    return 0;
}
