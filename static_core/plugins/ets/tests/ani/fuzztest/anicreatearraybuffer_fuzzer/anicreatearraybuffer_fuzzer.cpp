...
#include "ani.h"
#include "anicreatearraybuffer_fuzzer.h"
...
namespace OHOS {
void AniCreateArrayBufferFuzzTest(const char *data, size_t size)
{
    if (size <= 0U) {
        return;
    }
    ...
    auto bufferSize = provider.ConsumeIntegral<ani_size>();
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::AniCreateArrayBufferFuzzTest(reinterpret_cast<const char *>(data), size);
    return 0;
}
