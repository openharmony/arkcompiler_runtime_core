...
#include "anituplevaluegetitemshort_fuzzer.h"
...
namespace OHOS {
void AniTupleValueGetItemShortFuzzTest(const char *data, size_t size)
{
    if (size <= 0U) {
        return;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::AniTupleValueGetItemShortFuzzTest(reinterpret_cast<const char *>(data), size);
    return 0;
}
