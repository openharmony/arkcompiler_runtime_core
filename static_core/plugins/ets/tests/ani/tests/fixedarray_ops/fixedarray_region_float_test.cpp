...
class FixedArraySetGetRegionFloatTest : public AniTest {
protected:
...
// ninja ani_test_float_array_region_gtests
TEST_F(FixedArraySetGetRegionFloatTest, SetFloatArrayRegionErrorTests)
{
    ani_fixedarray_float fixedarray;
    ASSERT_EQ(env_->FixedArray_New_Float(5U, &fixedarray), ANI_OK);
    ...
    ASSERT_EQ(env_->FixedArray_SetRegion_Float(fixedarray, offset1, len1, nativeBuffer), ANI_OUT_OF_RANGE);

    const ani_size offset2 = 5;
    const ani_size len2 = 10U;
    ASSERT_EQ(env_->FixedArray_SetRegion_Float(fixedarray, offset2, len2, nativeBuffer), ANI_OUT_OF_RANGE);
    const ani_size offset3 = 0;
    const ani_size len3 = 5;
    ASSERT_EQ(env_->FixedArray_SetRegion_Float(fixedarray, offset3, len3, nativeBuffer), ANI_OK);
}
...
TEST_F(FixedArraySetGetRegionFloatTest, GetFloatArrayRegionErrorTests)
{
    ...
    ASSERT_EQ(env_->FixedArray_GetRegion_Float(fixedarray, offset1, len1, nullptr), ANI_INVALID_ARGS);
    const ani_size offset2 = 5;
    const ani_size len2 = 10U;
    ASSERT_EQ(env_->FixedArray_GetRegion_Float(fixedarray, offset2, len2, nativeBuffer), ANI_OUT_OF_RANGE);
    ASSERT_EQ(env_->FixedArray_GetRegion_Float(fixedarray, offset1, len1, nativeBuffer), ANI_OK);
}

TEST_F(FixedArraySetGetRegionFloatTest, GetRegionFloatTest)
{
    const auto array = static_cast<ani_fixedarray_float>(CallEtsFunction<ani_ref>("GetArray"));

    ani_float nativeBuffer[5U] = {0.0F};
    const ani_size offset3 = 0;
    const ani_size len3 = 5;
    const float epsilon = 1e-6;  // Define acceptable tolerance
    ASSERT_EQ(env_->FixedArray_GetRegion_Float(array, offset3, len3, nativeBuffer), ANI_OK);
...
TEST_F(FixedArraySetGetRegionFloatTest, SetRegionFloatTest)
{
    ...
    ASSERT_EQ(env_->FixedArray_SetRegion_Float(array, offset4, len4, nativeBuffer1), ANI_OK);
