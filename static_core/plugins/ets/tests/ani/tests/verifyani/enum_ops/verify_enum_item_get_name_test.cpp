...
class VerifyEnumItemGetNameTest : public VerifyAniTest {
public:
    void SetUp() override
    {
        VerifyAniTest::SetUp();
        ASSERT_EQ(env_->FindEnum("verify_enum_item_get_name_test.Color", &enumColor_), ANI_OK);
...
TEST_F(VerifyEnumItemGetNameTest, wrong_env)
{
    ...
    ASSERT_ERROR_ANI_ARGS_MSG("EnumItem_GetName", testLines);
}
...
TEST_F(VerifyEnumItemGetNameTest, wrong_enum_item_type_enum)
{
    ani_string result {};
    ASSERT_EQ(env_->EnumItem_GetName(reinterpret_cast<ani_enum_item>(enumColor_), &result), ANI_INVALID_ARGS);
    std::vector<TestLineInfo> testLines {
        {"env", "ani_env *"},
...
TEST_F(VerifyEnumItemGetNameTest, has_unhandled_error)
{
    ThrowError();
    ani_string result {};
...
TEST_F(VerifyEnumItemGetNameTest, cross_thread_enum_item_ref)
{
    ...
    std::thread([this, &crossThreadItem]() {
        ...
        ani_enum enm {};
        ASSERT_EQ(env->FindEnum("verify_enum_item_get_name_test.Color", &enm), ANI_OK);
        ASSERT_EQ(env->Enum_GetEnumItemByName(enm, "RED", &crossThreadItem), ANI_OK);
        ASSERT_EQ(vm_->DetachCurrentThread(), ANI_OK);
    }).join();

    ani_string result {};
    ASSERT_EQ(env_->EnumItem_GetName(crossThreadItem, &result), ANI_ERROR);
    std::vector<TestLineInfo> testLines {
        {"env", "ani_env *"},
        {"enum_item", "ani_enum_item", "wrong reference"},
        {"result", "ani_string *"},
...
TEST_F(VerifyEnumItemGetNameTest, success)
{
    ani_string result {};
    ASSERT_EQ(env_->EnumItem_GetName(itemColorRed_, &result), ANI_OK);
