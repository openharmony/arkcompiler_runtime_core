...
{
    ani_int result {};
    ASSERT_EQ(env_->EnumItem_GetValue_Int(reinterpret_cast<ani_enum_item>(enumColorInt_), &result), ANI_INVALID_TYPE);
    std::vector<TestLineInfo> testLines {
        {"env", "ani_env *"},
...
TEST_F(VerifyEnumItemGetValueIntTest, wrong_result_null)
{
    ...
    std::vector<TestLineInfo> testLines {
        {"env", "ani_env *"},
        {"enum_item", "ani_enum_item"},
        {"result", "ani_int *", "nullptr for storing 'ani_int' [ERROR]"},
    };
    ASSERT_ERROR_ANI_ARGS_MSG("EnumItem_GetValue_Int", testLines);
}
