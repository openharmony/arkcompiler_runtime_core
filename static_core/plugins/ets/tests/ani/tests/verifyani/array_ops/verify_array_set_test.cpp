...
class ArraySetTest : public VerifyAniTest {};
...
TEST_F(ArraySetTest, wrong_env)
{
    ...
    ASSERT_ERROR_ANI_ARGS_MSG("Array_Set", testLines);
}

TEST_F(ArraySetTest, wrong_input_array)
{
    ani_object intObj;
...
TEST_F(ArraySetTest, null_input_array)
{
    ani_array arr {};
...
TEST_F(ArraySetTest, undef_input_array)
{
    ani_array arr {};
...
TEST_F(ArraySetTest, err_input_array)
{
    ani_class cls {};
    ...
    CreateInt(env_, 1U, &intObj);
    ani_size index = 2U;
    ASSERT_EQ(env_->Array_Set(static_cast<ani_array>(errObj), index, intObj), ANI_ERROR);
    std::vector<TestLineInfo> testLines {
        {"env", "ani_env *"},
...
TEST_F(ArraySetTest, obj_input_array)
{
    ani_class cls {};
    ...
    CreateInt(env_, 1U, &intObj);
    ani_size index = 2U;
    ASSERT_EQ(env_->Array_Set(static_cast<ani_array>(obj), index, intObj), ANI_ERROR);
    std::vector<TestLineInfo> testLines {
        {"env", "ani_env *"},
...
TEST_F(ArraySetTest, str_input_array)
{
    ...
    CreateInt(env_, 1U, &intObj);
    ani_size index = 2U;
    ASSERT_EQ(env_->Array_Set(reinterpret_cast<ani_array>(stringObj), index, intObj), ANI_ERROR);
    std::vector<TestLineInfo> testLines {
        {"env", "ani_env *"},
...
TEST_F(ArraySetTest, throw_error)
{
    ani_array arr {};
    ani_size length = 3U;
    CreateArray(env_, length, &arr);
...
TEST_F(ArraySetTest, index_out_of_bounds)
{
    ani_array arr {};
    ani_size length = 3U;
    CreateArray(env_, length, &arr);
    ...
    ani_object intObj;
    CreateInt(env_, 1U, &intObj);
    ASSERT_EQ(env_->Array_Set(arr, length, intObj), ANI_PENDING_ERROR);
}

TEST_F(ArraySetTest, index_too_large)
{
    ani_array arr {};
    ani_size length = 3U;
    CreateArray(env_, length, &arr);
    ...
    ani_object intObj;
    CreateInt(env_, 1U, &intObj);
    ASSERT_EQ(env_->Array_Set(arr, SIZE_MAX, intObj), ANI_PENDING_ERROR);
}

TEST_F(ArraySetTest, success)
{
    ani_array arr {};
    ani_size length = 3U;
    CreateArray(env_, length, &arr);
    ...
    for (ani_size i = 0U; i < length; ++i) {
        ani_object intObj;
        CreateInt(env_, i, &intObj);
        ASSERT_EQ(env_->Array_Set(arr, i, intObj), ANI_OK);
    }
}
