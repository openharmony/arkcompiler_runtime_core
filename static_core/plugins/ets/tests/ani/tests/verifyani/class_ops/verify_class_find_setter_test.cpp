...
TEST_F(ClassFindSetterTest, lookup_status_is_forwarded_without_verify_abort)
{
    ...
    std::vector<TestLineInfo> testLines {
        {"env", "ani_env *"},
        {"class", "ani_class"},
        {"name", "const char *", "setter not found [ERROR]"},
        {"result", "ani_method *"},
    };
...
TEST_F(ClassFindSetterTest, wrong_class)
{
    ...
    std::vector<TestLineInfo> testLines {
        ...
        {"name", "const char *", "wrong class for setter [ERROR]"},
        {"result", "ani_method *"},
    };
    ASSERT_ERROR_ANI_ARGS_MSG("Class_FindSetter", testLines);
}
...
{
    ani_method method {};
    ASSERT_EQ(env_->c_api->Class_FindSetter(env_, class_, nullptr, &method), ANI_INVALID_ARGS);
    std::vector<TestLineInfo> testLines {
        {"env", "ani_env *"},
        {"class", "ani_class"},
...
{
    ASSERT_EQ(env_->c_api->Class_FindSetter(env_, class_, "prop", nullptr), ANI_INVALID_ARGS);
    std::vector<TestLineInfo> testLines {
        {"env", "ani_env *"},
        {"class", "ani_class"},
        {"name", "const char *"},
