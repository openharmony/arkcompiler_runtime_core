...
public:
    void GetMethodData(ani_class *clsResult, ani_static_method *methodResult)
    {
        ...
        ASSERT_EQ(env_->Class_GetStaticMethod(cls, "sum", "SS:S", &method), ANI_OK);
...
TEST_F(CallStaticMethodTest, call_static_method_short)
{
    ...
    ani_short sum;
    ASSERT_EQ(env_->c_api->Class_CallStaticMethod_Short(env_, cls, method, &sum, 5U, 6U), ANI_OK);
    ASSERT_EQ(sum, 5U + 6U);
}
...
TEST_F(CallStaticMethodTest, call_static_method_short_v)
{
    ...
    ani_short sum;
    ASSERT_EQ(env_->Class_CallStaticMethod_Short(cls, method, &sum, 5U, 6U), ANI_OK);
    ASSERT_EQ(sum, 5U + 6U);
}
...
TEST_F(CallStaticMethodTest, call_static_method_short_null_class)
{
    ...
    ani_short sum;
    ASSERT_EQ(env_->Class_CallStaticMethod_Short(nullptr, method, &sum, 5U, 6U), ANI_INVALID_ARGS);
}
...
TEST_F(CallStaticMethodTest, call_static_method_short_null_method)
{
    ...
    ani_short sum;
    ASSERT_EQ(env_->Class_CallStaticMethod_Short(cls, nullptr, &sum, 5U, 6U), ANI_INVALID_ARGS);
}
...
TEST_F(CallStaticMethodTest, call_static_method_short_null_result)
{
    ...
    ASSERT_EQ(env_->Class_CallStaticMethod_Short(cls, method, nullptr, 5U, 6U), ANI_INVALID_ARGS);
}
