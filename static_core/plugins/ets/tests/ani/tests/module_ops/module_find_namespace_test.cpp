...
public:
    void NamespaceFind(ani_namespace ns, ani_env *env)
    {
        ...
        ani_enum aniEnum {};
        ASSERT_EQ(env->Namespace_FindEnum(ns, "Color", &aniEnum), ANI_OK);
        ani_class kclass {};
        ASSERT_EQ(env->Namespace_FindClass(ns, "A", &kclass), ANI_OK);
        ASSERT_NE(variable, nullptr);

        ani_namespace ns1 {};
        ASSERT_EQ(env->Namespace_FindNamespace(ns, "ttt", &ns1), ANI_OK);
...
TEST_F(ModuleFindNamespaceTest, find_namespace)
{
    ...
    ASSERT_EQ(env_->Module_FindNamespace(module, "outer.inner", &ns), ANI_OK);
    ASSERT_NE(ns, nullptr);
    ASSERT_EQ(env_->Module_FindNamespace(module, "opst", &ns), ANI_OK);
    ASSERT_NE(ns, nullptr);
}
...
TEST_F(ModuleFindNamespaceTest, invalid_descriptor)
{
    ...
    ASSERT_EQ(ns, ns1);
    ASSERT_EQ(env_->Module_FindNamespace(module, "module_find_class_test.inner", &ns), ANI_NOT_FOUND);
    ASSERT_EQ(ns, ns1);
}
...
{
    ani_module module {};
    ASSERT_EQ(env_->FindModule("@abcModule.module_find_namespaceaaa_test", &module), ANI_NOT_FOUND);
...
TEST_F(ModuleFindNamespaceTest, invalid_env)
{
    ...
    ASSERT_NE(module, nullptr);
    ani_namespace ns {};
    ASSERT_EQ(env_->c_api->Module_FindNamespace(nullptr, module, "ops", &ns), ANI_INVALID_ARGS);
    ASSERT_EQ(ns, nullptr);
}
...
TEST_F(ModuleFindNamespaceTest, invalid_arg_result)
{
    ...
    ASSERT_EQ(env_->FindModule("@abcModule.module_find_namespace_test", &module), ANI_OK);
    ASSERT_NE(module, nullptr);
    ASSERT_EQ(env_->Module_FindNamespace(module, "module_find_class_test.atest", nullptr), ANI_INVALID_ARGS);
}
