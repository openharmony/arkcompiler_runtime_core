...
TEST_F(ObjectSetFieldByNameByteTest, set_field01)
{
    ...
    ASSERT_EQ(CallEtsFunction<ani_boolean>("checkObjectField", animal, static_cast<ani_byte>(SET_VALUE)), ANI_TRUE);

    ASSERT_EQ(env_->Object_SetFieldByName_Byte(animal, "value", static_cast<ani_byte>(0)), ANI_OK);
    ASSERT_EQ(CallEtsFunction<ani_boolean>("checkObjectField", animal, static_cast<ani_byte>(0)), ANI_TRUE);
...
TEST_F(ObjectSetFieldByNameByteTest, set_field02)
{
    ...
    for (int i = 1; i <= loopCount; i++) {
        ...
        ASSERT_EQ(CallEtsFunction<ani_boolean>("checkObjectField", animal, static_cast<ani_byte>(CMP_VALUE)), ANI_TRUE);
