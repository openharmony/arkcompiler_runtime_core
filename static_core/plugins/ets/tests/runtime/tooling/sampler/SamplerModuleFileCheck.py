...
class ModulesDumpTest():
    ...
    def module_file_check(self):
        ...
        sampler_test_abc_path = os.path.join(self._binary_dir, "plugins", "ets", "tests", "runtime", "tooling", "sampler", "SamplerTest.abc")
        ...
        with open(self._file_name, 'r') as my_file:
            while line := my_file.readline():
                ...
                if (checksum != expected_checksum):
                    print("SamplerModuleFileCheck: for file", pathname, "checksum is not equal", expected_checksum, "vs", checksum)
                    return False
...
def main():
    ...
    if test.module_file_check() != True:
