...
class EtsAnyTypeAPITest : public testing::Test {
public:
    EtsAnyTypeAPITest()
    {
        ...
        if (stdlib == nullptr) {
            std::cerr << "PANDA_STD_LIB env variable should be set and point to etstdlib.abc" << std::endl;
            std::abort();
        }
    ...
    ~EtsAnyTypeAPITest() override
    {
        Runtime::Destroy();
    }

    NO_COPY_SEMANTIC(EtsAnyTypeAPITest);
    NO_MOVE_SEMANTIC(EtsAnyTypeAPITest);
...
TEST_F(EtsAnyTypeAPITest, AnyTypeBaseClass)
{
    auto *anyClass = vm_->GetClassLinker()->GetClass("LY;");
    ASSERT(anyClass != nullptr);
    ASSERT_EQ(anyClass->GetBase(), nullptr);
}

TEST_F(EtsAnyTypeAPITest, AnyTypeMethods)
{
    auto *anyClass = vm_->GetClassLinker()->GetClass("LY;");
    ASSERT(anyClass != nullptr);
    auto methods = anyClass->GetMethods();
    ASSERT_EQ(methods.size(), 0);
}

TEST_F(EtsAnyTypeAPITest, AnyTypeFields)
{
    auto *anyClass = vm_->GetClassLinker()->GetClass("LY;");
    ASSERT(anyClass != nullptr);
    auto fields = anyClass->GetRuntimeClass()->GetInstanceFields();
