...
class EtsNullTypeAPITest : public testing::Test {
public:
    EtsNullTypeAPITest()
    {
    ...
    ~EtsNullTypeAPITest() override
    {
        Runtime::Destroy();
    }

    NO_COPY_SEMANTIC(EtsNullTypeAPITest);
    NO_MOVE_SEMANTIC(EtsNullTypeAPITest);
...
TEST_F(EtsNullTypeAPITest, NullTypeBaseClass)
{
    auto *nullClass = vm_->GetClassLinker()->GetClass("Lstd/core/Null;");
    ASSERT(nullClass != nullptr);
    ASSERT_EQ(nullClass->GetBase(), nullptr);
}

TEST_F(EtsNullTypeAPITest, NullTypeMethods)
{
    auto *nullClass = vm_->GetClassLinker()->GetClass("Lstd/core/Null;");
    ASSERT(nullClass != nullptr);
    auto methods = nullClass->GetMethods();
    ASSERT_EQ(methods.size(), 0);
}

TEST_F(EtsNullTypeAPITest, NullTypeFields)
{
    auto *nullClass = vm_->GetClassLinker()->GetClass("Lstd/core/Null;");
    ASSERT(nullClass != nullptr);
    auto fields = nullClass->GetRuntimeClass()->GetInstanceFields();
