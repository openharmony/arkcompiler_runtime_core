...
class EtsStdCoreArrayTest : public testing::Test {
public:
    EtsStdCoreArrayTest()
    {
    ...
    ~EtsStdCoreArrayTest() override
    {
        Runtime::Destroy();
    }

    NO_COPY_SEMANTIC(EtsStdCoreArrayTest);
    NO_MOVE_SEMANTIC(EtsStdCoreArrayTest);

    static std::vector<MirrorFieldInfo> GetMembers()
    {
        return std::vector<MirrorFieldInfo> {MIRROR_FIELD_INFO(EtsStdCoreArray, actualLength_, "actualLength"),
                                             MIRROR_FIELD_INFO(EtsStdCoreArray, buffer_, "buffer")};
    }
...
TEST_F(EtsStdCoreArrayTest, MemoryLayout)
{
    EtsClass *klass = PlatformTypes(vm_)->coreArray;
    MirrorFieldInfo::CompareMemberOffsets(klass, GetMembers());
}

TEST_F(EtsStdCoreArrayTest, EtsStdCoreArrayCreate)
{
    ...
    ASSERT_TRUE(array->IsExactlyStdCoreArray(EtsExecutionContext::FromMT(coro)));

    ASSERT_EQ(array->GetActualLengthFromStdCoreArray(), ARRAY_LENGTH);

    auto *dataArray = array->GetDataFromStdCoreArray();
...
TEST_F(EtsStdCoreArrayTest, GetRef)
{
    auto *coro = EtsCoroutine::GetCurrent();
    ...
    EtsHandle array(executionCtx, EtsStdCoreArray::Create(executionCtx, ARRAY_LENGTH));
    ASSERT_NE(array.GetPtr(), nullptr);

    EtsHandle dataArray(executionCtx, array->GetDataFromStdCoreArray());
...
TEST_F(EtsStdCoreArrayTest, StdCoreArrayGet)
{
    auto *coro = EtsCoroutine::GetCurrent();
    auto *executionCtx = EtsExecutionContext::FromMT(coro);
    ScopedManagedCodeThread s(coro);
    ...
    for (size_t i = 0; i < ARRAY_LENGTH; ++i) {
        ...
        // Check that `GetRef` works correctly
        EtsObject *result = array->StdCoreArrayGet(i);
        CheckObjectIsString(coro, result, sampleString);
    }
}

TEST_F(EtsStdCoreArrayTest, GetRefWithError)
{
    auto *coro = EtsCoroutine::GetCurrent();
    auto *executionCtx = EtsExecutionContext::FromMT(coro);
    ScopedManagedCodeThread s(coro);
...
TEST_F(EtsStdCoreArrayTest, SetRef)
{
    auto *coro = EtsCoroutine::GetCurrent();
    auto *executionCtx = EtsExecutionContext::FromMT(coro);
    ScopedManagedCodeThread s(coro);
...
TEST_F(EtsStdCoreArrayTest, StdCoreArraySet)
{
    auto *coro = EtsCoroutine::GetCurrent();
    auto *executionCtx = EtsExecutionContext::FromMT(coro);
    ScopedManagedCodeThread s(coro);
    ...
    EtsHandle dataArray(EtsExecutionContext::FromMT(coro), array->GetDataFromStdCoreArray());
    for (size_t i = 0; i < ARRAY_LENGTH; ++i) {
        const auto sampleString = std::to_string(i);
        auto *expectedString = EtsString::CreateFromUtf8(sampleString.c_str(), sampleString.size())->AsObject();
        array->StdCoreArraySet(i, expectedString);
        ASSERT_FALSE(coro->HasPendingException());
...
TEST_F(EtsStdCoreArrayTest, SetRefWithError)
{
    auto *coro = EtsCoroutine::GetCurrent();
    auto *executionCtx = EtsExecutionContext::FromMT(coro);
    ScopedManagedCodeThread s(coro);
