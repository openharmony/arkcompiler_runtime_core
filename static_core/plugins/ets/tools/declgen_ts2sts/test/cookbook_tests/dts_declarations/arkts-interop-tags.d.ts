/*
 * Copyright (c) 2026 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

export declare class Base {}
export declare interface IBase {}

// --- @noninterop: removes the declaration entirely ---

/** @noninterop */
export declare class RemovedClass {}

/** @noninterop */
export declare interface RemovedInterface {}

/** @noninterop */
export declare enum RemovedEnum { A = 0 }

/** @noninterop */
export declare function removedFunc(): void;

/** @noninterop */
export declare var removedVar: string;

/** @noninterop */
export declare type RemovedAlias = string;

// @noninterop on individual class members
export declare class ClassWithNoninteropMembers {
    keptMethod(): string;
    /** @noninterop */
    removedMethod(): void;
    keptProp: number;
    /** @noninterop */
    removedProp: string;
}

// @noninterop on individual interface members
export declare interface IWithNoninteropMembers {
    keptMethod(): string;
    /** @noninterop */
    removedMethod(): void;
    keptProp: number;
    /** @noninterop */
    removedProp: string;
}

// @noninterop on getter/setter/constructor
export declare class ClassWithAccessors {
    get keptProp(): string;
    /** @noninterop */
    get removedGetter(): string;
    /** @noninterop */
    set removedSetter(v: string);
    /** @noninterop */
    constructor(x: number);
}

// @noninterop on individual enum members
export declare enum PartialEnum {
    KeptA = 0,
    /** @noninterop */
    RemovedB = 1,
    KeptC = 2
}

// @noninterop on call/construct/index signatures
export declare interface IWithSignatures {
    keptProp: string;
    /** @noninterop */
    (x: number): void;
    /** @noninterop */
    new(x: number): IWithSignatures;
    /** @noninterop */
    [key: string]: string;
}

// --- @interop any: replaces class/interface/enum with `type Name = Any;` ---

/** @interop any */
export declare class AnyClass {
    method(): void;
}

/** @interop any */
export declare interface AnyInterface {
    prop: string;
}

/** @interop any */
export declare enum AnyEnum {
    A = 0,
    B = 1
}

// --- @interop any with generics: type parameters are fully preserved (constraints and defaults kept) ---

/** @interop any */
export declare class AnyGenericClass<T> {
    method(): void
}

/** @interop any */
export declare interface AnyGenericInterface<T, U> {
    prop: T;
}

/** @interop any */
export declare class AnyConstrainedClass<T extends Base> {
    method(): void
}

/** @interop any */
export declare class AnyDefaultClass<T = Base> {
    method(): void
}

// --- @interop break-extends: strips the extends heritage clause ---

/** @interop break-extends */
export declare class BreakExtendsClass extends Base {}

/** @interop break-extends */
export declare interface BreakExtendsInterface extends IBase {}

// --- @interop break-implements: strips the implements heritage clause ---

/** @interop break-implements */
export declare class BreakImplementsClass implements IBase {}

// --- @interop ret: changes the return type ---

/** @interop ret string */
export declare function retFunc(x: number): void;

// --- @interop param: changes a parameter type ---

/** @interop param 0 string */
export declare function paramFunc(x: number): void;

// --- class members using @interop ret and @interop param ---

export declare class Methods {
    /** @interop ret number */
    getValue(): string;
    /** @interop param 1 string */
    process(a: number, b: boolean): void;
    /** @interop param 0 string */
    constructor(x: number);
}

// --- interface method signatures using @interop ret and @interop param ---

export declare interface IMethodSigs {
    /** @interop ret number */
    getValue(): string;
    /** @interop param 1 string */
    process(a: number, b: boolean): void;
}
