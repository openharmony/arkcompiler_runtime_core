...
|LANG| does not support keyof as the main purpose of keyof is to use the object
as a set of properties model in |TS| and acess the object properties by name.
|LANG| resolves access to object members by name at compile-time and generates 
efficient code. So, if access by name is required during the program execution
reflextion is to be used instead. keyof is to be replaced with the union type
created from the names of properties.
...
.. code-block:: typescript
    ...
    class Derived extends Base {
        override f2() {}
        f3 = ():void => { console.log ("f3 from Derived is called") }
    }
...
.. code-block:: typescript
    ...
    x = "f3" // OK
    let d = new Derived
    d.f3() // Just call what is required
    // If the access is to be perfomed dynamically use reflection
