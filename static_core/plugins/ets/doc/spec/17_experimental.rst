...
*Value array type* is the built-in type written as ``ValueArray<T>`` and
characterized by the following: 
...
.. note:: 
    ...
    - ``ValueArray<T>`` is not a generic type, despite using notation identical
      to generics.
...
*Value array* can be created by using :ref:`Array Literal` or constructors
defined for type ``ValueArray<T>`` (see below).
...
If it is to be used in a *type call expression*, then its first parameter must
be a *factory* defined as a parameterless function type returning the class
...
parameter is passed implicitly and its value is generated by the compiler as
the ``factory`` lambda, which creates new instance of the type specified in the
*type call expression*:
...
An implicitly passed factory returns an instance of the class used in the call:
...
.. code-block:: typescript
   :linenos:
    ...
    let a = A()    // Creates instance of 'A', declared type of 'a' is A
    let b = B()    // Creates instance of 'B', declared type of 'b' is A
    console.log(a) // Output: A{}
    console.log(b) // Output: B{field: 1}
    
    console.log(b.field) // Compile-time error, declared type of 'b' is 'A'

Where the method ``$_instantiate`` is used implicitly in the
*type call expression*:

- If the method ``$_instantiate`` does not declare ``factory`` as an
  *optional parameter*, then a ``factory`` implementation is generated
  automatically.

- If the method ``$_instantiate`` declares ``factory`` as an
  *optional parameter* (see :ref:`Optional Parameters`), then the provided
  default implementation is used for ``factory``.
...
A *type call expression* passes no arguments to the ``factory`` function, and
the latter uses a parameterless class constructor. If a class has no
parameterless class constructor, then a :index:`compile-time error` occurs:
...
..  note::
    Calling the method ``$_instantiate`` explicitly with such a class, or
    supplying the default implementation for the factory that uses a
    constructor with parameters, is still possible though useless:
...
Otherwise, a :index:`compile-time error` occurs. The erroneous
...
.. code-block:: typescript
   :linenos:
    ...
    class E implements I1, I2 {
        overload foo { f2, f4 } // Compile-time error, not all methods are used
    }
...
.. code-block:: typescript
   :linenos:
    ...
    interface I3 extends I1, I2 {
       // Compile-time error, no new overload for 'foo' is defined
    }
    interface I4 extends I1, I2 {
        overload foo { f4, f1, f3, f2 } // OK, as new overload is defined
    }
    interface I5 extends I1, I2 {
        overload foo { f1, f3 } // Compile-time error, not all methods are included
    }
...
.. code-block:: typescript
   :linenos:
      ...
      function bar(this: A) { ...
         ...
         this.member_1 // Compile-time error, member_1 is not accessible
         this.member_2 // Compile-time error, member_2 is not accessible
         ...
      }
      let a = new A()
...
.. code-block:: typescript
   ...
   // Compile-time error, cannot assign entity with method call syntax
   // to a function type
   x.foo(x.bar)
   x.foo(x.bar<int>)
   let z = x.bar
...
To call *lambda with receiver* one must use :ref:`Function Call Expression`
where the first argument is the receiver object. 
...
.. code-block:: typescript
   :linenos:
    ...
    function foo(aa: A[], f: (this: A) => void) {
        for (let a of aa) {
            f(a) // ordinary lambda call
        }
    }

    let aa: A[] = [{name: "aa"}, {name: "bb"}]
...
*Lambda with receiver* cannot be called using :ref:`Method Call Expression`.
Attempting to do so causes a :index:`compile-time error`:
...
.. code-block:: typescript
   :linenos:
    ...
    function foo(a: A, f: (this: A) => void) {
        a.f() // Compile-time error, lambda cannot be called in this way
    }
...
.. note::
   ...
   not to the surrounding class or interface. 
   ...
   .. code-block:: typescript
      :linenos:
         ...
         new A().lambda (new B) // Output is 'foo() from B is called'
...
.. code-block:: typescript
   :linenos:
      ...
      goo() { console.log("aa") } // Compile-time error, goo() requires an argument
      goo(); { console.log("aa") } // Compile-time error, goo() requires an argument
