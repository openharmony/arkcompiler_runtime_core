...
:ref:`Type Alias Declaration`), then compiler can replace the type alias for a
...
.. note::
   ...
   of arbitrary precision. Also it is not a value type. That is why it is
   discussed in detail in the appropriate subsection (see :ref:`Type bigint`).
...
-  :ref:`Type never`;
-  :ref:`Type null`;
-  :ref:`Type undefined or void`; and
-  :ref:`Type Parameters`.
...
supertype of :ref:`Type undefined or void` and :ref:`Type null` in particular.
...
(see :ref:`Subtyping`) of all types except :ref:`Type undefined or void`,
...
- Type of variables, class fields, or interface properties that never get
  a value (however, an assignment statement with type ``never`` in both
  left-hand and right-hand sides is valid).
- Type of parameters of a function or a method, or a constructor to prevent the
  body of that function or method, or constructor from being executed.

.. code-block:: typescript
   :linenos:

    function foo (): never {
       // any call to 'foo' never returns
    ...
    class C implements I {
        prop: never = foo() // assignment is valid
    }
    new C // instance creation will lead to a runtime error generated by 'foo()'
...
Type ``undefined`` or ``void``
******************************

.. meta:
    frontend_status: Done

Type ``undefined`` has a single value named ``undefined`` (see
:ref:`Undefined Literal`).
Type ``void`` is the alias for type ``undefined``, so it has the same
single value named ``undefined``.
The use of types ``undefined`` and ``void`` is interchangeable.

Type alias ``void`` is used typically as a return type to highlight that a
...
.. code-block:: typescript
   :linenos:
    ...
    function f1 (): void {
        return // OK, return with no expression
    }
...
Both ``undefined`` and ``void`` can be used as a type argument that instantiates a generic
...
Type ``bigint`` does not belong to the hierarchy of :ref:`Numeric Types`.
...
- Binary arithmetic expressions that use an operand of type ``bigint`` along
  with an operand of another type are illegal and produce a
  :index:`compile-time error`.
- The equality expression with ``bigint`` against non-``bigint``
  causes a :index:`compile-time warning`. It always evaluates as
  ``false`` for operators ``'=='``, ``'==='``, and
  ``true`` for operators ``'!='``, ``'!=='``.

An exception is relational operators that use an operand of type ``bigint``
along with an operand of another type. For details, see
:ref:`Bigint Relational Operators`.
...
.. code-block:: typescript
   ...
   // One may assign the variable of the union type with any valid value
    ...
    type Union1 = string | StringEnum // OK
...
#. Identical types (see :ref:`Type Identity`) within a union type are replaced
   for a single type with account to the ``readonly`` type flag priority.
