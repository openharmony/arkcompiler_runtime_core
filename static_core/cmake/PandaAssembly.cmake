...
function(add_panda_assembly)
    ...
    set_target_properties(${ARG_TARGET} PROPERTIES FILE "${binary_file}")
endfunction()
...
function(panda_add_asm_file)
    ...
    set(panda_cli    ark)
    set(context     "${ARG_LANGUAGE_CONTEXT}")
    set(stdlibs)

    if ("${context}" STREQUAL "core")
        list(APPEND stdlibs "${${panda_stdlib}_BINARY_DIR}/${panda_stdlib}.abc")
    endif()
    ...
    if (ARG_AOT_MODE)
        ...
        set(backend_compiler "aot")
        if (DEFINED ARG_AOT_COMPILER)
            set(backend_compiler ${ARG_AOT_COMPILER})
        endif()
        ...
        set(launcher_aot
            ...
            "--paoc-mode=${backend_compiler}"
            "--boot-panda-files \"${boot_stdlibs}\""
            "--load-runtimes=${context}"
    ...
    if (ARG_RUNTIME_FAIL_TEST)
        string(APPEND launcher " || (exit 0)")
    endif()
    ...
    if ("${context}" STREQUAL "core")
        add_dependencies(${ARG_TARGET} ${panda_stdlib})
    endif()
