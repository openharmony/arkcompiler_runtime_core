...
struct is_vectorable : public std::false_type {
};

template <class V>
struct is_vectorable<V, std::void_t<typename V::value_type, typename V::allocator_type,
                                    decltype(std::declval<V>().size()), decltype(std::declval<V>().data())>>
    : public std::bool_constant<is_random_access_iterable_v<V>> {
};
...
struct is_stringable : public std::false_type {
};

template <class S>
struct is_stringable<S, std::void_t<typename S::value_type, typename S::allocator_type, typename S::traits_type,
...
struct is_hash_mappable : public std::false_type {
};

template <class HM>
struct is_hash_mappable<
