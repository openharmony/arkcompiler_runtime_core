...
#ifndef PANDA_LIBPANDABASE_UTILS_TYPE_CONVERTER_H
#define PANDA_LIBPANDABASE_UTILS_TYPE_CONVERTER_H
...
public:
    ...
    PANDA_PUBLIC_API double GetDoubleValue() const;

    uint64_t GetUint64Value() const;

    PANDA_PUBLIC_API std::string_view GetLiteral() const;
...
PANDA_PUBLIC_API std::ostream &operator<<(std::ostream &os, const ValueUnit &element);
 ...
 *  @param bytes
 */
PANDA_PUBLIC_API ValueUnit MemoryConverter(uint64_t bytes);
...
#endif  // PANDA_LIBPANDABASE_UTILS_TYPE_CONVERTER_H
