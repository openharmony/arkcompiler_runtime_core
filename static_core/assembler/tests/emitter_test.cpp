...
TEST(emittertests, novalue_return_type)
{
    Parser p;

    auto source = R"(
        .function novalue primitiveNeverNoParams() <static, access.function=public> {
            return.void
        }
    )";
    auto res = p.Parse(source);
    ...
    size_t numMethods = 0;
    const auto neverPrim = panda_file::Type(panda_file::Type::TypeId::NOVALUE);
    cda.EnumerateMethods([&](panda_file::MethodDataAccessor &mda) {
        panda_file::ProtoDataAccessor pda(*pf, mda.GetProtoId());
        ASSERT_EQ(neverPrim, pda.GetReturnType());
        ASSERT_EQ(0, pda.GetNumArgs());
        ASSERT_TRUE(pda.GetReturnType().IsPrimitive());
        ++numMethods;
    });
    ASSERT_EQ(1, numMethods);
}

TEST(emittertests, novalue_with_ref_never_param)
{
    Parser p;
    auto source = R"(
        .record N <external>
    ...
    panda_file::ClassDataAccessor cda(*pf, classId);
    const auto neverPrim = panda_file::Type(panda_file::Type::TypeId::NOVALUE);
    const auto ref = panda_file::Type(panda_file::Type::TypeId::REFERENCE);
    ...
    cda.EnumerateMethods([&](panda_file::MethodDataAccessor &mda) {
        ...
        ASSERT_TRUE(pda.GetReturnType().IsPrimitive());
        ASSERT_EQ(1, pda.GetNumArgs());
        ASSERT_EQ(ref, pda.GetArgType(0));
        ASSERT_FALSE(pda.GetArgType(0).IsPrimitive());
        ++numMethods;
    });
    ASSERT_EQ(1, numMethods);
}

TEST(emittertests, novalue_mixed_signatures)
{
    Parser p;
    auto source = R"(
        .record N <external>
        .function novalue foo1() <static, access.function=public> { return.void }
        .function novalue foo2(i32 a0) <static, access.function=public> { return.void }
        .function novalue foo3(N a0, i32 a1) <static, access.function=public> { return.void }
        .function novalue foo4(f64 a0, f64 a1) <static, access.function=public> { return.void }
    )";
    auto res = p.Parse(source);
    ASSERT_EQ(p.ShowError().err, Error::ErrorType::ERR_NONE);
    ...
    ASSERT_EQ(4, numMethods);
}

TEST(emittertests, novalue_shorty_encoding)
{
    Parser p;
    auto source = R"(
        .function novalue foo(i32 a0) <static, access.function=public> {
            return.void
        }
    )";
    auto res = p.Parse(source);
    ...
    cda.EnumerateMethods([&](panda_file::MethodDataAccessor &mda) {
        panda_file::ProtoDataAccessor pda(*pf, mda.GetProtoId());
        auto retType = pda.GetReturnType();
        ASSERT_EQ(retType.GetId(), panda_file::Type::TypeId::NOVALUE);
        ASSERT_EQ(retType.GetEncoding(), 0xf);
        ASSERT_EQ(pda.GetArgType(0).GetId(), panda_file::Type::TypeId::I32);
        ASSERT_EQ(pda.GetNumArgs(), 1);
    });
}
