...
TEST_F(SchedulerTest, FixSSInBBAfterScheduler)
{
    GRAPH(GetGraph())
    {
        ...
        BASIC_BLOCK(2, -1)
        {
            ...
            INST(5, Opcode::LoadAndInitClass).ref().Inputs(4).TypeId(68);
            INST(6, Opcode::SaveState).NoVregs();
            INST(7, Opcode::LoadConstArray).ref().Inputs(6);
            INST(8, Opcode::Add).s32().Inputs(1, 2);
            // LoadArray will move on one position down
            INST(9, Opcode::LoadArray).s32().Inputs(7, 8);
            INST(10, Opcode::SaveState).NoVregs();

            INST(11, Opcode::NOP);
            INST(12, Opcode::Return).s32().Inputs(9);
        }
    }
    ...
    GRAPH(graph)
    {
        ...
        BASIC_BLOCK(2, -1)
        {
            ...
            INST(10, Opcode::SaveState).Inputs(7).SrcVregs({VirtualRegister::BRIDGE});
            INST(9, Opcode::LoadArray).s32().Inputs(7, 8);
