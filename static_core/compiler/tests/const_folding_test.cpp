...
public:
    ...
    void CastTest(From src, To dst, DataType::Type dst_type)
    {
        ...
        GRAPH(graph)
        {
            ...
            BASIC_BLOCK(2, 1)
            {
                ...
                INS(1).SetType(dst_type);
                INST(2, Opcode::Return).Inputs(1);
                INS(2).SetType(dst_type);
            }
        }
                                   ...
                                   ConditionCode cc, uint64_t result)
    {
        ...
        GRAPH(graph)
        {
            BASIC_BLOCK(2, 1)
            {
                INST(0, Opcode::LoadImmediate).ref().Class(class1);
                INST(1, Opcode::LoadImmediate).ref().Class(class2);
                INST(2, Opcode::Compare).b().SrcType(DataType::Type::REFERENCE).CC(cc).Inputs(0, 1);
...
TEST_F(ConstFoldingTest, CompareLoadImmediateTest)
{
    ...
    CheckCompareLoadImmediate(class1, class1, ConditionCode::CC_EQ, 1);
    CheckCompareLoadImmediate(class1, class2, ConditionCode::CC_EQ, 0);
    CheckCompareLoadImmediate(class1, class1, ConditionCode::CC_NE, 0);
    CheckCompareLoadImmediate(class1, class2, ConditionCode::CC_NE, 1);
}
