...
TEST_F(BasicBlockTest, IfLikelyUnlikelyTest)
{
    ...
    GRAPH(graph1)
    {
        ...
        BASIC_BLOCK(2, 3, 4)
        {
            ...
            INST(3, Opcode::IfImm).SrcType(DataType::Type::BOOL).CC(CC_NE).Inputs(2).Likely();
        }
        BASIC_BLOCK(3, 5)
        {
            INST(4, Opcode::Sub).s64().Inputs(0, 1);
        }
        BASIC_BLOCK(4, 5)
        {
            INST(5, Opcode::Sub).s64().Inputs(1, 0);
        }
        BASIC_BLOCK(5, -1)
        {
            INST(6, Opcode::Phi).s64().Inputs(4, 5);
    ...
    ASSERT_EQ(&BB(2), blocks1.at(1));
    ASSERT_EQ(&BB(3), blocks1.at(2));
    ASSERT_EQ(&BB(5), blocks1.at(3));
    ...
    GRAPH(graph2)
    {
        ...
        {
            INST(2, Opcode::Compare).b().SrcType(DataType::Type::INT64).CC(CC_GT).Inputs(0, 1);
            INST(3, Opcode::IfImm).SrcType(DataType::Type::BOOL).CC(CC_NE).Inputs(2).Unlikely();
        }
        BASIC_BLOCK(3, 5)
        {
            INST(4, Opcode::Sub).s64().Inputs(0, 1);
    ...
    ASSERT_EQ(&BB(2), blocks2.at(1));
    ASSERT_EQ(&BB(4), blocks2.at(2));
    ASSERT_EQ(&BB(5), blocks2.at(3));
    ASSERT_EQ(&BB(3), blocks2.back());
}
