...
TEST_F(LoweringTest, IfRef)
{
    ...
    GRAPH(graph)
    {
        ...
        BASIC_BLOCK(4, 5, 6)
        {
            INST(5, Opcode::Phi).u64().Inputs(1, 2);
            INST(6, Opcode::SaveState).NoVregs();
            INST(7, Opcode::CallStatic).v0id().InputsAutoType(5, 6);
            INST(8, Opcode::IfImm).SrcType(DataType::BOOL).CC(CC_NE).Imm(0).Inputs(3);
        }
        BASIC_BLOCK(5, -1)
        {
            INST(9, Opcode::ReturnI).s32().Imm(1);
        }
        BASIC_BLOCK(6, -1)
        {
            INST(10, Opcode::ReturnI).s32().Imm(1);
        }
    }
    ...
    GRAPH(graph_if)
    {
        ...
        {
            INST(3, Opcode::Compare).b().CC(CC_NE).Inputs(0, 1);
            INST(4, Opcode::IfImm).SrcType(DataType::REFERENCE).CC(CC_NE).Imm(0).Inputs(0);
        }
        BASIC_BLOCK(3, 4) {}
        BASIC_BLOCK(4, 5, 6)
        {
            INST(5, Opcode::Phi).u64().Inputs(1, 2);
            INST(6, Opcode::SaveState).Inputs(0).SrcVregs({VirtualRegister::BRIDGE});
            INST(7, Opcode::CallStatic).v0id().InputsAutoType(5, 6);
            INST(8, Opcode::IfImm).SrcType(DataType::REFERENCE).CC(CC_NE).Imm(0).Inputs(0);
        }
        BASIC_BLOCK(5, -1)
        {
            INST(9, Opcode::ReturnI).s32().Imm(1);
...
{
    std::array<std::array<ConditionCode, 2>, 2> codes = {
    ...
    for (auto cc : codes) {
        ...
        GRAPH(graph)
        {
            ...
            BASIC_BLOCK(2, 3, 4)
            {
                INST(4, Opcode::And).b().Inputs(0, 1);
                INST(5, Opcode::IfImm).SrcType(DataType::BOOL).CC(cc[0]).Imm(0).Inputs(4);
            }
            BASIC_BLOCK(3, -1)
            {
                INST(6, Opcode::Return).b().Inputs(3);
        ...
        GRAPH(graph_if)
        {
            ...
            BASIC_BLOCK(2, 3, 4)
            {
                INST(2, Opcode::If).SrcType(DataType::BOOL).CC(cc[1]).Inputs(0, 1);
            }
            BASIC_BLOCK(3, -1)
            {
                INST(3, Opcode::ReturnI).b().Imm(1);
...
TEST_F(LoweringTest, SignedModPowerOfTwo)
{
    ...
    GRAPH(graph1)
    {
        ...
        BASIC_BLOCK(2, 1)
        {
            INST(2, Opcode::Mod).s64().Inputs(0, 1);
            INST(3, Opcode::Return).s64().Inputs(2);
        }
    }
    ...
    if (GetGraph()->GetArch() == Arch::AARCH32) {
        ...
        {
            PARAMETER(0, 0).i64();
            CONSTANT(7, 0xfffffffffffffffc);
            BASIC_BLOCK(2, 1)
            {
                INST(4, Opcode::AddI).s64().Imm(3).Inputs(0);
                INST(5, Opcode::SelectImm).s64().CC(CC_LT).Imm(0).Inputs(4, 0, 0);
                INST(6, Opcode::And).s64().Inputs(5, 7);
                INST(8, Opcode::Sub).s64().Inputs(0, 6);
                INST(3, Opcode::Return).s64().Inputs(8);
            }
        }
    } else {
        GRAPH(graph2)
        {
            ...
            BASIC_BLOCK(2, 1)
            {
                INST(4, Opcode::AddI).s64().Imm(3).Inputs(0);
                INST(5, Opcode::SelectImm).s64().CC(CC_LT).Imm(0).Inputs(4, 0, 0);
                INST(6, Opcode::AndI).s64().Imm(0xfffffffffffffffc).Inputs(5);
                INST(7, Opcode::Sub).s64().Inputs(0, 6);
                INST(3, Opcode::Return).s64().Inputs(7);
            }
        }
    }
...
TEST_F(LoweringTest, UnsignedModPowerOfTwo)
{
    ...
    GRAPH(graph2)
    {
        ...
        BASIC_BLOCK(2, 1)
        {
            INST(4, Opcode::AndI).u64().Imm(3).Inputs(0);
            INST(3, Opcode::Return).u64().Inputs(4);
        }
    }
    ASSERT_TRUE(graph1->RunPass<Lowering>());
    graph1->RunPass<Cleanup>();
...
{
    std::array<ConditionCode, 3> codes {ConditionCode::CC_NE, ConditionCode::CC_EQ, ConditionCode::CC_BE};
    for (auto code : codes) {
        for (auto swap : {true, false}) {
            ...
            GRAPH(graph)
            {
                ...
                BASIC_BLOCK(2, 1)
                {
                    ...
                    INST(3, Opcode::CallStatic).b().InputsAutoType(2);
                    if (swap) {
                        INST(4, Opcode::Compare).b().Inputs(1, 3).CC(code);
                    } else {
                        INST(4, Opcode::Compare).b().Inputs(3, 1).CC(code);
                    }
            ...
            if (code == ConditionCode::CC_EQ) {
                ...
                GRAPH(graph_final)
                {
                    BASIC_BLOCK(2, 1)
                    {
                        INST(2, Opcode::SaveState).NoVregs();
                        INST(3, Opcode::CallStatic).b().InputsAutoType(2);
                        INST(8, Opcode::XorI).b().Inputs(3).Imm(1);
                        INST(5, Opcode::Return).b().Inputs(8);
                    }
                }
