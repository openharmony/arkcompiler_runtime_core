...
private:
    ...
    void TryRemoveFromSaveStates(Inst *inst)
    {
        ...
        for (auto userIt = inst->GetUsers().begin(); userIt != inst->GetUsers().end();) {
            ...
            if (!userInst->IsSaveState() ||
                (userInst->IsSaveStateSuspend() && userIt->GetVirtualRegister().IsBridge()) ||
                userInst->GetBasicBlock()->IsMarked(marker_) || userInst->IsMarked(marker_) ||
                userIt->GetVirtualRegister().IsEnv()) {
                ++userIt;
            } else {
