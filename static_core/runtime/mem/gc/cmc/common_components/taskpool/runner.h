...
class Runner {
public:
    explicit Runner(uint32_t threadNum, const std::function<void(ark::os::thread::NativeHandleType)> prologueHook,
    ...
    void PANDA_PUBLIC_API TerminateThread();
    ...
    {
        ark::os::memory::LockHolder guard(mtxPool_);
    ...
    void PrologueHook(ark::os::thread::NativeHandleType thread)
    {
        if (prologueHook_ != nullptr) {
            prologueHook_(thread);
        }
    }
    void EpilogueHook(ark::os::thread::NativeHandleType thread)
    {
