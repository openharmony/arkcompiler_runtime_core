...
public:
    ...
    inline bool IsResurrectedObject(const BaseObject *obj) const
    {
        return RegionalHeap::IsResurrectedObject(obj);
    }
    ...
    void TransitionToGCPhase(const GCPhase phase, const bool)
    {
        ...
        Heap::GetHeap().GetCollector().NotifyGCListeners(
            [currentPhase](GCListener *l) { l->OnGCPhaseEnd(currentPhase); });
        MutatorManager::Instance().TransitionAllMutatorsToGCPhase(phase);
        Heap::GetHeap().GetCollector().NotifyGCListeners([phase](GCListener *l) { l->OnGCPhaseStart(phase); });
    }

    GCStats &GetGCStats() override
    {
...
protected:
    ...
    inline void SetGCReason(const GCReason reason)
    {
        gcReason_ = reason;
    }
