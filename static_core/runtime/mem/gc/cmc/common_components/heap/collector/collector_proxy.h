...
#ifndef COMMON_RUNTIME_COMMON_COMPONENTS_HEAP_COLLECTOR_COLLECTOR_PROXY_H
#define COMMON_RUNTIME_COMMON_COMPONENTS_HEAP_COLLECTOR_COLLECTOR_PROXY_H
...
public:
    explicit CollectorProxy(Allocator &allocator, CollectorResources &resources) : arkCollector_(allocator, resources)
    {
    ...
    {
        return currentCollector_->GetGCPhase();
    }

    void SetGCPhase(const GCPhase phase) override
    {
        currentCollector_->SetGCPhase(phase);
    }

    // dispatch garbage collection to the right collector
    PANDA_PUBLIC_API void RunGarbageCollection(uint64_t gcIndex, GCReason reason, GCType gcType) override;

    bool ShouldIgnoreRequest(GCRequest &request) override
    {
        return currentCollector_->ShouldIgnoreRequest(request);
    }

    MarkingCollector &GetCurrentCollector() const
    {
        return *currentCollector_;
    }

    BaseObject *FindToVersion(BaseObject *obj) const override
    {
        return currentCollector_->FindToVersion(obj);
    }

    bool IsOldPointer(RefField<> &ref) const override
    {
        return currentCollector_->IsOldPointer(ref);
    }
    bool IsCurrentPointer(RefField<> &ref) const override
    {
        return currentCollector_->IsCurrentPointer(ref);
    }

    bool IsFromObject(BaseObject *obj) const override
    {
        return currentCollector_->IsFromObject(obj);
    }
    ...
    {
        return currentCollector_->IsUnmovableFromObject(obj);
    }

    BaseObject *ForwardObject(BaseObject *obj) override
    {
        return currentCollector_->ForwardObject(obj);
    }
