...
public:
    ...
    bool MarkCardTable(size_t offset)
    {
        ...
        // Atomic with relaxed order reason: data race with card table with no synchronization or ordering constraints
        // imposed on other reads or writes
        bool isMarked = ((card->load(std::memory_order_relaxed) & headMaskBits) != 0);
        if (!isMarked) {
            // Atomic with relaxed order reason: data race with card table with no synchronization or ordering
    ...
    void VisitAllMarkedCardBefore(const std::function<void(BaseObject *)> &func, HeapAddress regionStart,
                                  HeapAddress end)
    {
