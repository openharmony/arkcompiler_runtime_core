...
struct RegionBitmap {
    ...
    {
        CHECK(regionSize % (kMarkedBytesPerBit * kBitsPerWord) == 0);
    ...
    bool MarkBits(size_t start)
    {
        ...
        // Atomic with relaxed order reason: data race with markWords during concurrent marking with no
        // synchronization or ordering constraints imposed on other reads or writes
        uint64_t old = markWords[headWordIdx].load(std::memory_order_relaxed);
        bool isMarked = ((old & headMaskBits) != 0);
        if (!isMarked) {
            // Atomic with relaxed order reason: data race with markWords during concurrent marking with no
            // synchronization or ordering constraints imposed on other reads or writes
            old = markWords[headWordIdx].fetch_or(headMaskBits, std::memory_order_relaxed);
    ...
    bool IsMarked(size_t start)
    {
        ...
        // Atomic with relaxed order reason: data race with markWords during concurrent marking with no
        // synchronization or ordering constraints imposed on other reads or writes
        bool ret = ((markWords[headWordIdx].load(std::memory_order_relaxed) & mask) != 0);
        return ret;
    }
};
}  // namespace ark::common_vm
