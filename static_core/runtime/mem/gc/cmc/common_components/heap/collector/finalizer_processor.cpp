...
FinalizerProcessor::FinalizerProcessor()
{
    ...
    // Atomic with relaxed order reason: data race with shouldReclaimHeapGarbage_ with no synchronization or ordering
    // constraints imposed on other reads or writes
    shouldReclaimHeapGarbage_.store(false, std::memory_order_relaxed);
    // Atomic with relaxed order reason: data race with shouldFeedHungryBuffers_ with no synchronization or ordering
    // constraints imposed on other reads or writes
...
void FinalizerProcessor::Run()
{
    ...
    while (running_) {
        ...
        // Atomic with relaxed order reason: data race with shouldFeedHungryBuffers_ with no synchronization or
        // ordering constraints imposed on other reads or writes
        if (shouldFeedHungryBuffers_.load(std::memory_order_relaxed)) {
            FeedHungryBuffers();
        }

        // Atomic with relaxed order reason: data race with shouldReclaimHeapGarbage_ with no synchronization or
        // ordering constraints imposed on other reads or writes
...

        {
            ...
            while (running_) {
                ...
                // Atomic with relaxed order reason: data race with job flags with no synchronization or ordering
                // constraints imposed on other reads or writes
                auto hasFinalizableJob = hasFinalizableJob_.load(std::memory_order_relaxed);
                // Atomic with relaxed order reason: data race with job flags with no synchronization or ordering
                // constraints imposed on other reads or writes
                auto shouldReclaimHeapGarbage = shouldReclaimHeapGarbage_.load(std::memory_order_relaxed);
                // Atomic with relaxed order reason: data race with job flags with no synchronization or ordering
                // constraints imposed on other reads or writes
                auto shouldFeedHungryBuffers = shouldFeedHungryBuffers_.load(std::memory_order_relaxed);
                if (hasFinalizableJob || shouldReclaimHeapGarbage || shouldFeedHungryBuffers) {
                    break;
                }
            }
        }
...
{
    wakeCondition_.SignalAll();
}

void FinalizerProcessor::Wait(uint32_t timeoutMilliSeconds)
{
    ark::os::memory::LockHolder lock(wakeLock_);
    wakeCondition_.TimedWait(&wakeLock_, timeoutMilliSeconds);
}
...
void FinalizerProcessor::WaitStarted()
{
    ark::os::memory::LockHolder lock(startedLock_);
    while (!started_) {
        startedCondition_.Wait(&startedLock_);
    }
}
