...
public:
    ...
    static void SetStartupStatus(StartupStatus status)
    {
        ...
        startupStatus_.store(status, std::memory_order_relaxed);
    }
...
public:
    ...
    {
        // Atomic with release order reason: data race with sensitiveStatus_ with dependecies on writes before
        // the store
    ...
    {
        // Atomic with acquire order reason: data race with sensitiveStatus_ with dependecies on reads after the load
    ...
    {
        // Atomic with seq_cst order reason: data race with sensitiveStatus_ with requirement for sequentially
    ...
    {
        // Atomic with release order reason: data race with recordSizeBeforeSensitive_ with dependecies on writes
        // before the store
    ...
    {
        // Atomic with acquire order reason: data race with recordSizeBeforeSensitive_ with dependecies on reads
        // after the load
