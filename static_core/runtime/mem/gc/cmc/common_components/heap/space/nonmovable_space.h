 ...
 */
#ifndef COMMON_RUNTIME_COMMON_COMPONENTS_HEAP_SPACE_NONMOVABLE_SPACE_H
#define COMMON_RUNTIME_COMMON_COMPONENTS_HEAP_SPACE_NONMOVABLE_SPACE_H
...
public:
    ...
    {
        size_t nonMovableUnitCount = polySizeRegionList_.GetUnitCount() + recentPolySizeRegionList_.GetUnitCount();
        for (size_t i = 0; i < NONMOVABLE_OBJECT_SIZE_COUNT; i++) {
    ...
    void ClearRSet()
    {
        auto clearFunc = [](RegionDesc *region) { region->ClearRSet(); };
