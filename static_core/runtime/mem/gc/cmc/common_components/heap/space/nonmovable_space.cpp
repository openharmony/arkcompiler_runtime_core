...
uintptr_t NonMovableSpace::Alloc(size_t size, bool allowGC)
{
    ...
    ark::os::memory::Mutex &listMutex = list->GetListMutex();
    ark::os::memory::LockHolder lock(listMutex);
    ...
    if (addr == 0) {
        RegionDesc *region = regionManager_.TakeRegion(1, RegionDesc::UnitRole::SMALL_SIZED_UNITS, false, allowGC);
        if (region == nullptr) {
            return 0;
        }
...
{
    uintptr_t addr = 0;
    ark::os::memory::Mutex &regionListMutex = recentPolySizeRegionList_.GetListMutex();

    ark::os::memory::LockHolder lock(regionListMutex);
...
uintptr_t NonMovableSpace::AllocFullRegion()
{
    ...
    [[maybe_unused]] uintptr_t addr = region->Alloc(region->GetRegionEnd() - region->GetRegionAllocPtr());
    DCHECK(addr != 0);
...
{
    auto visitFunc = [&func](RegionDesc *region) { region->VisitRememberSetBeforeMarking(func); };
