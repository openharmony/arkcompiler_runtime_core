 ...
 */
#ifndef COMMON_RUNTIME_COMMON_COMPONENTS_HEAP_SPACE_TO_SPACE_H
#define COMMON_RUNTIME_COMMON_COMPONENTS_HEAP_SPACE_TO_SPACE_H
...
public:
    ToSpace(RegionManager &regionManager)
        : RegionalSpace(regionManager), tlToRegionList_("thread local to-regions"), fullToRegionList_("full to-regions")
    {
    }
    ...
    {
        DCHECK(Heap::GetHeap().IsGcStarted());
    ...
    RegionList &GetTlToRegionList() noexcept
    {
        return tlToRegionList_;
    }

    RegionList &GetFullToRegionList() noexcept
    {
        return fullToRegionList_;
    }
