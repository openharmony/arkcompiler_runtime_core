...
public:
    ...
    RegionDesc *GetPreparedRegion()
    {
        // Atomic with acquire order reason: data race with preparedRegion_ with dependecies on reads after the load
        return preparedRegion_.load(std::memory_order_acquire);
    }
    ...
    bool SetPreparedRegion(RegionDesc *newPreparedRegion)
    {
        ...
        // Atomic with release/relaxed order reason: data race with preparedRegion_ with dependecies on writes before
    ...
    {
        stackRoots_.emplace_back(root);
    }

    // record stack roots in allocBuffer so that mutator can concurrently enumerate roots without lock.
    void PushRoot(uint64_t *root)
    {
        taggedObjStackRoots_.emplace_back(root);
    }
    ...
    {
        return MEMBER_OFFSET(AllocationBuffer, tlRegion_);
    }

    static constexpr size_t GetTLOldRegionOffset()
    {
        return MEMBER_OFFSET(AllocationBuffer, tlOldRegion_);
    }

    static constexpr size_t GetTLABOffset()
    {
        return MEMBER_OFFSET(AllocationBuffer, tlab_);
    }
