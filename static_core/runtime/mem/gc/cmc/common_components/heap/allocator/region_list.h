...
public:
    ...
    {
        CHECK((nRegion <= std::numeric_limits<size_t>::max() - regionCount_) &&
              (nUnit <= std::numeric_limits<size_t>::max() - unitCount_));
        regionCount_ += nRegion;
        unitCount_ += nUnit;
    }

    RegionDesc *GetHeadRegion() const
    {
        return listHead_;
    }
    ...
    {
        ark::os::memory::LockHolder lg(listMutex_);
    ...
    ark::os::memory::Mutex &GetListMutex()
    {
        return listMutex_;
    }
...
protected:
    ark::os::memory::Mutex listMutex_;
...
private:
    ...
    {
        size_t allocCnt = 0;
        ark::os::memory::LockHolder lock(const_cast<RegionList *>(this)->listMutex_);
