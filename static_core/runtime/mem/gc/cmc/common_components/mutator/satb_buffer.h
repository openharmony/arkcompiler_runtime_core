...
#ifndef COMMON_RUNTIME_COMMON_COMPONENTS_MUTATOR_SATB_BUFFER_H
#define COMMON_RUNTIME_COMMON_COMPONENTS_MUTATOR_SATB_BUFFER_H
...
public:
    ...
    public:
        ...
        bool IsEmpty() const
        {
            return reinterpret_cast<size_t>(top_) == reinterpret_cast<size_t>(container_);
        }
        bool IsFull() const
        {
    ...
    public:
        ...
        void Push(T *n)
        {
            // Atomic with relaxed order reason: data race with head_ with no synchronization or ordering constraints
            // imposed on other reads or writes
            T *old = head_.load(std::memory_order_relaxed);
            do {
                n->next = old;
                // Atomic with release/relaxed order reason: data race with head_ with dependecies on writes before
                // the store on success and relaxed on failure
        ...
        T *Pop()
        {
            ...
            do {
                ...
                !head_.compare_exchange_weak(old, old->next, std::memory_order_release, std::memory_order_relaxed));
            old->next = nullptr;
            return old;
        }
    ...
    public:
        ...
        void Reset()
        {
            ark::os::memory::LockHolder lg(safeLock_);
            head_ = nullptr;
        }
    ...
    void RetireNode(TreapNode *node) noexcept
    {
        retiredNodes_.Push(node);
    }
