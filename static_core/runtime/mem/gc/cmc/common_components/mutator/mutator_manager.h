...
public:
    ...
    {
        // Atomic with acquire order reason: data race with worldStopped_ with dependecies on reads after the load
        return worldStopped_.load(std::memory_order_acquire);
    }

    bool StwTriggered() const
    {
        // Atomic with seq_cst order reason: data race with stwTriggered_ with requirement for sequentially
        // consistent order where threads observe all modifications in the same order
        return stwTriggered_.load(std::memory_order_seq_cst);
    }

    void LockStopTheWorld() ACQUIRE(stwMutex_)
    {
        stwMutex_.Lock();
    }

    void UnlockStopTheWorld() noexcept RELEASE(stwMutex_)
    {
        stwMutex_.Unlock();
    }
    ...
    {
        // Atomic with acquire order reason: data race with suspensionMutatorCount_ with dependecies on reads after
        // the load
    ...
    {
        // Atomic with release order reason: data race with suspensionMutatorCount_ with dependecies on writes before
        // the store
    ...
    {
        ark::os::memory::LockHolder mutatorsToStopLck(mutatorSuspensionMtx_);
        while (GetSuspensionMutatorCount() != 0) {
            mutatorSuspensionCV_.Wait(&mutatorSuspensionMtx_);
        }
    }
    ...
    {
        ark::os::memory::LockHolder mutatorsToStopLck(mutatorSuspensionMtx_);
        mutatorSuspensionCV_.SignalAll();
    }
#endif

#if not defined(_WIN64)
    const SafepointPageManager *GetSafepointPageManager() const
    {
        return safepointPageManager_;
    }
#endif
    ...
    ark::os::memory::Mutex allMutatorListLock_; /* Mutex guard for allMutatorList */
...
#if defined(_WIN64) || defined(__APPLE__)
    ark::os::memory::ConditionVariable mutatorSuspensionCV_;
    ark::os::memory::Mutex mutatorSuspensionMtx_;
#endif
#if not defined(_WIN64)
...
class ScopedSTWLock {
public:
    __attribute__((always_inline)) explicit ScopedSTWLock()
    {
        MutatorManager::Instance().LockStopTheWorld();
    }

    __attribute__((always_inline)) ~ScopedSTWLock()
    {
