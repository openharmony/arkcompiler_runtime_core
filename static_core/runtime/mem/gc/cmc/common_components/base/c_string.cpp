...
const char *CString::Str() const noexcept
{
    return str_;
}

char *CString::GetStr() const noexcept
{
    return str_;
}
...
char *CString::BaseName(const CString &path)
{
    return basename(path.str_);
}
...
bool CString::ParseFlagFromEnv(const CString &s)
{
...
CString CString::RemoveBlankSpace() const
{
    ...
    DCHECK(noBlankSpaceStr.str_ != nullptr);
