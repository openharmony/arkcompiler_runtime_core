...
#ifndef COMMON_RUNTIME_COMMON_COMPONENTS_COMMON_WORK_STACK_H
#define COMMON_RUNTIME_COMMON_COMPONENTS_COMMON_WORK_STACK_H
...
protected:
    ...
    NO_COPY_SEMANTIC(LocalStackBaseWithoutNotify);
    NO_MOVE_SEMANTIC(LocalStackBaseWithoutNotify);
};

template <typename PushToGlobalNotify>
class LocalStackBaseWithNotify {
protected:
    explicit LocalStackBaseWithNotify(PushToGlobalNotify *pushToGlobalNotify) : pushToGlobalNotify_(pushToGlobalNotify)
    {
        DCHECK(pushToGlobalNotify_ != nullptr);
    }
    ...
    NO_COPY_SEMANTIC(LocalStackBaseWithNotify);
    NO_MOVE_SEMANTIC(LocalStackBaseWithNotify);
...
template <typename T, size_t capacity, typename PushToGlobalNotify>
class LocalStackImpl final
    : public std::conditional_t<std::is_same_v<DummyNoPushToGlobalNotify, PushToGlobalNotify>,
                                LocalStackBaseWithoutNotify, LocalStackBaseWithNotify<PushToGlobalNotify>> {
    using InternalStack = StackBase<T, capacity>;
...
public:
    template <typename U = PushToGlobalNotify, typename = std::enable_if<!std::is_same_v<DummyNoPushToGlobalNotify, U>>>
    ...
    NO_COPY_SEMANTIC(LocalStackImpl);
    NO_MOVE_SEMANTIC(LocalStackImpl);
