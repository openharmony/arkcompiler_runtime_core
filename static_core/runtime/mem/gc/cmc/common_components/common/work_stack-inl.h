...
{
    DCHECK(e != nullptr);
    if (UNLIKELY(inStack_->IsFull())) {
        PushInStackToGlobal();
    }
    DCHECK(!inStack_->IsFull());
    inStack_->Push(e);
}
...
{
    if (UNLIKELY(outStack_->IsEmpty())) {
        if (UNLIKELY(!inStack_->IsEmpty())) {
            std::swap(inStack_, outStack_);
        } else if (!PopOutStackFromGlobal()) {
            return false;
        }
    }
    DCHECK(!outStack_->IsEmpty());
    outStack_->Pop(e);
    return true;
}
