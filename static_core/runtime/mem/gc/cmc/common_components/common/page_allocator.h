...
public:
    ...
    ~AllocatorUtils() = delete;
    NO_COPY_SEMANTIC(AllocatorUtils);
    NO_MOVE_SEMANTIC(AllocatorUtils);
...
public:
    ...
    void *Allocate()
    {
        void *result = nullptr;
        {
            ark::os::memory::LockHolder guard(allocLock_);
    ...
    void Deallocate(void *slot)
    {
        ...
        ark::os::memory::LockHolder lockGuard(allocLock_);
...
private:
    ...
    Page *nonFull_;
    ark::os::memory::Mutex allocLock_;
...
public:
    ...
    NO_INLINE PANDA_PUBLIC_API static AggregateAllocator &Instance(AllocationTag tag);
    ...
    NO_INLINE void Deallocate(void *p, size_t size)
    {
        uint32_t alignedSize = AlignUp(static_cast<uint32_t>(size), AllocatorUtils::ALLOC_ALIGNMENT);
        if (alignedSize <= RUN_ALLOC_LARGE_SIZE) {
            uint32_t index = RUNTYPE_SIZE_TO_RUN_IDX(alignedSize);
