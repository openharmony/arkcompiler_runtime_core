...
#include "runtime/mem/gc/cmc/heap/collector/copy_data_manager.h"
#include "runtime/mem/gc/cmc/heap/collector/region_bitmap.h"
#include "runtime/mem/gc/cmc/heap/collector/region_rset.h"
#include "securec.h"
...
public:
    ...
    {
        // Atomic with acquire order reason: data race with fieldVal_ with dependecies on reads after the load
    ...
    void AtomicSetValue(size_t pos, size_t bitLen, T newValue)
    {
        do {
            ...
            // Atomic with acq_rel/acquire order reason: data race with fieldVal_ with acquire-release semantics on
...
public:
    ...
    {
        // Atomic with acquire order reason: data race with markBitmap_ with dependecies on reads after the load
    ...
    ALWAYS_INLINE RegionBitmap *GetOrAllocMarkBitmap()
    {
        do {
            ...
            // Atomic with seq_cst/relaxed order reason: data race with markBitmap_ with requirement for sequentially
            // consistent order on success and relaxed on failure
    ...
    {
        // Atomic with acquire order reason: data race with resurrectBitmap_ with dependecies on reads after the load
    ...
    {
        do {
            // Atomic with acquire order reason: data race with resurrectBitmap_ with dependecies on reads after
            // the load
            RegionBitmap *bitmap = __atomic_load_n(&metadata.liveInfo_.resurrectBitmap_, std::memory_order_acquire);
            if (UNLIKELY(reinterpret_cast<uintptr_t>(bitmap) == RegionLiveDesc::TEMPORARY_PTR)) {
                continue;
            ...
            RegionBitmap *newValue = reinterpret_cast<RegionBitmap *>(RegionLiveDesc::TEMPORARY_PTR);
            // Atomic with seq_cst/relaxed order reason: data race with resurrectBitmap_ with requirement for
    ...
    ALWAYS_INLINE size_t GetAddressOffset(HeapAddress address)
    {
        DCHECK(GetRegionBaseFast() <= address);
    ...
    static HeapAddress GetUnitAddress(size_t unitIdx)
    {
        return UnitInfo::GetUnitAddress(unitIdx);
    }
    ...
    {
        DCHECK(GetRegionEnd() > GetRegionBase());
        return GetRegionEnd() - GetRegionBase();
    }

    size_t GetUnitCount() const
    {
        return GetRegionBaseSize() / UNIT_SIZE;
    }
    ...
    size_t GetRegionAllocatedSize() const
    {
    ...
    {
        return static_cast<UnitRole>(metadata.unitRole);
    }
    ...
    {
        return RegionDesc::GetUnitAddress(GetUnitIdx());
    }
    ...
    HeapAddress GetRegionStart() const
    {
        return metadata.regionStart;
    }

    HeapAddress GetRegionEnd() const
    {
        return metadata.regionEnd;
    }

    void SetRegionAllocPtr(HeapAddress addr)
    {
        metadata.allocPtr = addr;
    }

    HeapAddress GetRegionAllocPtr() const
    {
        return metadata.allocPtr;
    }

    HeapAddress GetMarkingLine() const
    {
        return metadata.markingLine;
    }
    HeapAddress GetCopyLine() const
    {
        return metadata.forwardLine;
    }
    ...
    {
        // Atomic with acq_rel order reason: data race with allocPtr during concurrent allocation
        ...
        if (addr <= GetRegionEnd()) {
            // Atomic with seq_cst order reason: rollback allocPtr on allocation failure, must be visible to all
            // threads
    ...
    {
        ark::os::memory::LockHolder lg(metadata.regionMutex);
    ...
    {
        return GetRegionType() == RegionType::TO_REGION;
    }
    bool IsOldRegion() const
    {
        return GetRegionType() == RegionType::OLD_REGION;
    }

    bool IsGarbageRegion() const
    {
        return GetRegionType() == RegionType::GARBAGE_REGION;
    }
    bool IsFreeRegion() const
    {
        return static_cast<UnitRole>(metadata.unitRole) == UnitRole::FREE_UNITS;
    }
    ...
    {
        return MEMBER_OFFSET(UnitMetadata, allocPtr);
    }

    static constexpr size_t GetRegionEndOffset()
    {
        return MEMBER_OFFSET(UnitMetadata, regionEnd);
    }
...
private:
    ...
    struct UnitMetadata {
        ...
        ark::os::memory::Mutex regionMutex;
    };
    ...
    public:
        ...
        void SetUnitRole(UnitRole role)
        {
            metadata_.unitBits.AtomicSetValue(0, BITS_5, static_cast<uint8_t>(role));
        }
        ...
        UnitRole GetUnitRole() const
        {
            return static_cast<UnitRole>(metadata_.unitRole);
        }
