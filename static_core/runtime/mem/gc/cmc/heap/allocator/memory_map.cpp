 ...
 */
#include "runtime/mem/gc/cmc/heap/allocator/memory_map.h"
...
MemoryMap *MemoryMap::MapMemory(size_t reqSize, size_t initSize, const Option &opt)
{
    ...
    os::mem::TagAnonymousMemory(mappedAddr, reqSize, (PandaString("ArkTS Heap CMCGC ") + opt.tag).c_str());
    return memMap;
}
...
MemoryMap *MemoryMap::MapMemoryAlignInner4G(uint64_t reqSize, uint64_t initSize, const Option &opt)
{
    ...
    [[maybe_unused]] uintptr_t baseAddr = 0x0;
#ifdef PANDA_TARGET_64
    ...
    os::mem::TagAnonymousMemory(mappedAddr, needReqSize, (PandaString("ArkTS Heap CMCGC ") + opt.tag).c_str());
    return memMap;
}
