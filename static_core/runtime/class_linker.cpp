...
void ClassLinker::AddPandaFile(std::unique_ptr<const panda_file::File> &&pf, ClassLinkerContext *context)
{
    ...
    if (context != nullptr && !context->IsBootContext()) {
        GetAotManager()->AddProfiledPandaFile(*file);
    }

    if (context == nullptr || context->IsBootContext()) {
        os::memory::LockHolder lock {bootPandaFilesLock_};
        bootPandaFiles_.push_back(file);
        if (!bootClassIndex_.Empty()) {
            bootClassIndex_.Clear(allocator_);
        }
    }
...
void ClassLinker::BuildBootClassIndex()
{
    os::memory::LockHolder lock {bootPandaFilesLock_};
    BuildBootClassIndexLocked();
}

void ClassLinker::BuildBootClassIndexLocked()
{
    ClassIndex::Builder builder;

    for (uint32_t fileIdx = 0; fileIdx < bootPandaFiles_.size(); fileIdx++) {
        const panda_file::File *pf = bootPandaFiles_[fileIdx];
        LOG(DEBUG, CLASS_LINKER) << "boot panda file " << fileIdx << ": " << pf->GetFilename();
        auto classes = pf->GetClasses();

        for (uint32_t offset : classes) {
            panda_file::File::EntityId entityId {offset};
            if (pf->IsExternal(entityId)) {
                continue;
            }
            panda_file::File::StringData className = pf->GetStringData(entityId);
            builder.Add(className.data, std::make_pair(fileIdx, offset));
        }
    }

    builder.Build(bootClassIndex_, allocator_);
}
...
static ClassEntry FindClassInBootPandaFiles(const uint8_t *descriptor, const PandaFiles &pandaFiles,
                                            const ClassIndex &classIndex)
{
    if (!classIndex.Empty()) {
        auto idx = classIndex.Find(descriptor);
        if (idx == ClassIndex::INVALID_INDEX) {
            // name not in hash list
            return {};
        }

        auto *pf = pandaFiles[idx.first];
        panda_file::File::EntityId classId {idx.second};

        auto className = pf->GetStringData(classId);
        if (utf::CompareMUtf8ToMUtf8(descriptor, className.data) == 0) {
            return {classId, pf};
        }
        // hash matched but className mismatch, fallback to normal search
    }
...
static void LayoutFieldsWithoutAlignment(size_t size, size_t *offset, size_t *space, Span<Field *> &fields)
{
    auto lastProceededElement = fields.size();
    ...
    for (auto i = fields.begin(); i != fields.end(); i++) {
        if (!(space == nullptr || *space >= size)) {
            lastProceededElement = i - fields.begin();
            break;
        }
        Field *field = *i;
    ...
    fields = fields.SubSpan(lastProceededElement);
}

static uint32_t LayoutReferenceFields(size_t size, size_t *offset, Span<Field *> fields)
{
...
static size_t LayoutFieldsInBaseClassPadding(Class *klass, Span<Field *> fields64, Span<Field *> &fields32,
                                             Span<Field *> &fields16, Span<Field *> &fields8, Span<Field *> refFields,
                                             bool isStatic)
{
    size_t offset;
...
// CC-OFFNXT(G.FUN.01) solid logic
static size_t LayoutFields(Class *klass, Span<Field *> fields64, Span<Field *> fields32, Span<Field *> fields16,
                           Span<Field *> fields8, Span<Field *> refFields, bool isStatic)
{
    size_t offset = LayoutFieldsInBaseClassPadding(klass, fields64, fields32, fields16, fields8, refFields, isStatic);
    if (!refFields.empty()) {
    ...
    if (!IsAligned<SIZE_64>(offset) && !fields64.empty()) {
...
static inline bool PushBackFieldIfNonPrimitiveType(Field &field, Field **&refFields)
{
...
bool ClassLinker::LayoutFields(Class *klass, Span<Field> fields, bool isStatic, mem::InternalArenaAllocator &allocator,
                               [[maybe_unused]] ClassLinkerErrorHandler *errorHandler)
{
    Field **fields64 = allocator.AllocArray<Field *>(fields.size());
    Field **fields32 = allocator.AllocArray<Field *>(fields.size());
    Field **fields16 = allocator.AllocArray<Field *>(fields.size());
    Field **fields8 = allocator.AllocArray<Field *>(fields.size());
    Field **refFields = allocator.AllocArray<Field *>(fields.size());

    Field **fields64Ptr = fields64;
    Field **fields32Ptr = fields32;
    Field **fields16Ptr = fields16;
    Field **fields8Ptr = fields8;
    Field **refFieldsPtr = refFields;

    for (auto &field : fields) {
        if (PushBackFieldIfNonPrimitiveType(field, refFieldsPtr)) {
            continue;
        }
    ...
    size_t size = ark::LayoutFields(klass, {fields64, fields64Ptr}, {fields32, fields32Ptr}, {fields16, fields16Ptr},
                                    {fields8, fields8Ptr}, {refFields, refFieldsPtr}, isStatic);
...
bool ClassLinker::LinkFields(Class *klass, mem::InternalArenaAllocator &allocator,
                             ClassLinkerErrorHandler *errorHandler)
{
    if (!LayoutFields(klass, klass->GetStaticFields(), true, allocator, errorHandler)) {
        LOG(ERROR, CLASS_LINKER) << "Cannot layout static fields of class '" << klass->GetName() << "'";
        return false;
    }

    if (!LayoutFields(klass, klass->GetInstanceFields(), false, allocator, errorHandler)) {
                              ...
                              ClassLinkerExtension *ext, ClassLinkerErrorHandler *errorHandler)
{
    ...
    if (!LinkFields(klass, *classInfo.vtableBuilder->GetAllocator(), errorHandler)) {
                                           ...
                                           const uint8_t *descriptor)
{
    ...
    if (!LinkFields(klass, *classInfo->vtableBuilder->GetAllocator(), ext->GetErrorHandler())) {
                             ...
                             ClassLinkerErrorHandler *errorHandler /* = nullptr */)
{
    ...
    if (context->IsBootContext()) {
        ...
        const panda_file::File *pandaFile {nullptr};
        {
            {
                os::memory::LockHolder lock {bootPandaFilesLock_};
                std::tie(classId, pandaFile) = FindClassInBootPandaFiles(descriptor, bootPandaFiles_, bootClassIndex_);
            }
                             ...
                             ClassLinkerErrorHandler *errorHandler /* = nullptr */)
{
    ...
    if (context->IsBootContext()) {
        ...
        {
            os::memory::LockHolder lock {bootPandaFilesLock_};
            std::tie(extId, pfPtr) = FindClassInBootPandaFiles(descriptor, bootPandaFiles_, bootClassIndex_);
        }
