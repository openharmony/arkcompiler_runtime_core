...
{
    DCHECK(!IsTreeString());
    DCHECK(!str1->IsTreeString() && !str2->IsTreeString());
...
inline ObjectType BaseString::GetStringType() const
{
    ...
    DCHECK(IsStringType(type) && ("Invalid ObjectType"));
    return type;
}
...
void BaseString::WriteToFlat(ReadBarrier &&readBarrier, const BaseString *src, Char *buf, uint32_t maxLength)
{
    ...
    while (true) {
        DCHECK(length <= maxLength && length > 0);
        DCHECK(length <= src->GetLength());
                                    ...
                                    uint32_t pos)
{
    ...
    while (true) {
        DCHECK(length + pos <= maxLength && length > 0);
        DCHECK(length <= src->GetLength());
        DCHECK(pos >= 0);
        switch (src->GetStringType()) {
            case ObjectType::LINE_STRING: {
...
ark::common_vm::Span<const uint8_t> BaseString::ToUtf8Span(ReadBarrier &&readBarrier, Vec &buf, bool modify, bool cesu8)
{
    ...
    if (UNLIKELY(IsUtf16())) {
        ...
        DCHECK(UtfUtils::Utf16ToUtf8Size(data, strLen, modify, false, cesu8) > 0);
