...
public:
     ...
     */
    template <typename ReadBarrier>
    uint32_t PANDA_PUBLIC_API GetHashcode(ReadBarrier &&readBarrier);
     ...
     */
    template <typename T>
    static PANDA_PUBLIC_API uint32_t ComputeHashForData(const T *data, size_t size, uint32_t hashSeed);
...
{
    DCHECK(length < BaseString::MAX_STRING_LENGTH);
    uint32_t newVal = 0;
...
template <typename ReadBarrier>
uint32_t PANDA_PUBLIC_API BaseString::GetHashcode(ReadBarrier &&readBarrier)
{
    uint32_t hashcode = GetMixHashcode();
