...
public:
    ...
    StackfulCoroutineWorker(Runtime *runtime, PandaVM *vm, StackfulCoroutineManager *coroManager, ScheduleLoopType type,
                            PandaString name, Id id, bool inExclusiveMode, bool isMainWorker);
    ~StackfulCoroutineWorker() override = default;

    static StackfulCoroutineWorker *FromJobWorkerThread(JobWorkerThread *worker)
    {
        return static_cast<StackfulCoroutineWorker *>(worker);
    }
    ...
    /// @brief Add created coroutine to waiters or to runnables if event happened
    void AddCoroutineWaitingForStart(JobEvent *startEvent, Coroutine *coro);
...
private:  // data members
    ...
    PriorityQueue<Coroutine> runnables_ GUARDED_BY(runnablesLock_);
    // blocked coros-related members: Coroutine AWAITS JobEvent
    mutable os::memory::Mutex waitersLock_;
    PandaMap<JobEvent *, Coroutine *> waiters_ GUARDED_BY(waitersLock_);
