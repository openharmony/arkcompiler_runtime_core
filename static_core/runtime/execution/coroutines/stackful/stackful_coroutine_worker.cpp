...
StackfulCoroutineWorker::StackfulCoroutineWorker(Runtime *runtime, PandaVM *vm, StackfulCoroutineManager *coroManager,
                                                 ScheduleLoopType type, PandaString name, Id id, bool inExclusiveMode,
                                                 bool isMainWorker)
      ...
      stats_(std::move(name))
{
    ...
    if (type == ScheduleLoopType::THREAD) {
        ...
        auto *schedulerLoopCtx = coroManager->CreateNativeCoroutine(
            GetRuntime(), GetPandaVM(), ScheduleLoopProxy, this, "[fiber_sch] " + GetName(), Coroutine::Type::SCHEDULER,
            CoroutinePriority::MEDIUM_PRIORITY);
        SetSchedulerExecutionCtx(schedulerLoopCtx);
        ASSERT(schedulerLoopCtx != nullptr);
        AddRunnableCoroutine(schedulerLoopCtx);
    }
}
...
void StackfulCoroutineWorker::WaitForEvent(JobEvent *awaitee)
{
    ...
    if (!RunnableCoroutinesExist()) {
        ...
        LOG(FATAL, COROUTINES) << coroManager_->GetAllWorkerFullStatus()->OutputInfo();
        UNREACHABLE();
    }
...
void StackfulCoroutineWorker::AddCoroutineWaitingForStart(JobEvent *startEvent, Coroutine *coro)
{
    ASSERT(startEvent != nullptr);
    ASSERT(coro != nullptr);
    ASSERT(coro != Coroutine::GetCurrent());

    startEvent->Lock();
    if (startEvent->Happened()) {
        startEvent->Unlock();
    ...
    }
    {
        ...
        [[maybe_unused]] auto [_, isInserted] = waiters_.emplace(startEvent, coro);
        ASSERT(isInserted);
    }
}

void StackfulCoroutineWorker::UnblockWaiters(JobEvent *blocker)
{
...
void StackfulCoroutineWorker::FinalizeFiberScheduleLoop()
{
    ...
    if (RunnableCoroutinesExist()) {
        ...
        ASSERT(GetSchedulerExecutionCtx()->GetJob()->HasNativeEntrypoint());
        runnablesLock_.Lock();
        // sch loop only
...
void StackfulCoroutineWorker::PrintRunnables(const PandaString &requester)
{
    ...
    runnables_.IterateOverElements([](Coroutine *co) { LOG(DEBUG, COROUTINES) << co->GetName() << " <"; });
...
size_t StackfulCoroutineWorker::GetRunnablesCount(Coroutine::Type type)
{
    ...
    runnables_.IterateOverElements([type, &runnablesCount](Coroutine *coro) {
...
void StackfulCoroutineWorker::ThreadProc()
{
    ...
    SetSchedulerExecutionCtx(schedulerLoopCtx);
    ASSERT(schedulerLoopCtx != nullptr);
    Coroutine::SetCurrent(schedulerLoopCtx);
    schedulerLoopCtx->RequestResume();
    AddRunningCoroutine(schedulerLoopCtx);
    schedulerLoopCtx->NativeCodeBegin();
...
{
    using CIterator = PriorityQueue<Coroutine>::CIterator;
    PandaVector<CIterator> migratedCoros;
    runnables_.VisitElements([&migratedCoros, &migrateCount, this, to](auto begin, auto end) {
    ...
    runnables_.RemoveElements(migratedCoros);
}
...
void StackfulCoroutineWorker::CacheLocalObjectsInExecutionCtx()
{
    os::memory::LockHolder lock(runnablesLock_);
    runnables_.IterateOverElements([](Coroutine *co) { co->UpdateCachedObjects(); });
    {
...
void StackfulCoroutineWorker::GetFullWorkerStateInfo(StackfulCoroutineWorkerStateInfo *info) const
{
    ...
    {
        os::memory::LockHolder lock(runnablesLock_);
        runnables_.IterateOverElements([&info](Coroutine *co) { info->AddCoroutine(co); });
    }
}
