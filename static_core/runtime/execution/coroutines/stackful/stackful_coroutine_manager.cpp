...
{
    auto *wMain =
        CreateWorker(runtime, vm, StackfulCoroutineWorker::ScheduleLoopType::FIBER, "[main] worker ", false, true);
...
void StackfulCoroutineManager::CreateGeneralWorkers(size_t howMany, Runtime *runtime, PandaVM *vm)
{
...
void StackfulCoroutineManager::FinalizeGeneralWorkers(size_t howMany, Runtime *runtime, PandaVM *vm)
{
...
StackfulCoroutineWorker *StackfulCoroutineManager::CreateWorker(Runtime *runtime, PandaVM *vm,
                                                                ...
                                                                PandaString workerName, bool inExclusiveMode,
                                                                bool isMainWorker)
{
...
void StackfulCoroutineManager::OnWorkerShutdown(JobWorkerThread *worker)
{
...
void StackfulCoroutineManager::OnWorkerStartup(JobWorkerThread *worker)
{
    worker->OnBeforeWorkerStartup(createEtsObjFunc_);
    os::memory::LockHolder lock(workersLock_);
    OnWorkerStartupImpl(StackfulCoroutineWorker::FromJobWorkerThread(worker));
}
...
void StackfulCoroutineManager::DisableJobSwitch()
{
    GetCurrentWorker()->DisableCoroutineSwitch();
}

void StackfulCoroutineManager::EnableJobSwitch()
{
    GetCurrentWorker()->EnableCoroutineSwitch();
}

bool StackfulCoroutineManager::IsJobSwitchDisabled()
{
...
bool StackfulCoroutineManager::TerminateCoroutine(Coroutine *co)
{
    if (co->GetJob()->HasManagedEntrypoint()) {
        ...
        GetCurrentWorker()->GetPerfStats().StartInterval(JobTimeStats::SCH_ALL);
    } else {
    ...
    } else if (co->GetJob()->HasNativeEntrypoint()) {
...
void StackfulCoroutineManager::Await(JobEvent *awaitee)
{
    ASSERT_NATIVE_CODE();
    // profiling
    ScopedJobStats s(&GetCurrentWorker()->GetPerfStats(), JobTimeStats::SCH_ALL);
...
void StackfulCoroutineManager::UnblockWaiters(JobEvent *blocker)
{
    ...
    ScopedJobStats s(&GetCurrentWorker()->GetPerfStats(), JobTimeStats::SCH_ALL, true);
...
bool StackfulCoroutineManager::EnumerateJobsImpl(const EnumerateJobsCallback &cb) const
{
    os::memory::LockHolder lh(coroListLock_);
...
void StackfulCoroutineManager::ReuseCoroutineInstance(Coroutine *co, Job *job)
{
    auto *ctx = co->GetContext<CoroutineContext>();
    co->ReInitialize(job, ctx);
}
...
{
    ASSERT(co != nullptr);
    auto maskValue = co->GetJob()->GetAffinityMask();
    return ChooseWorkerImpl(WorkerSelectionPolicy::LEAST_LOADED, maskValue);
}

AffinityMask StackfulCoroutineManager::CalcAffinityMask(const JobWorkerThreadGroup::Id &groupId)
{
    ASSERT(groupId != JobWorkerThreadGroup::InvalidId());
    ...
    switch (GetSchedulingPolicy()) {
        case CoroutineSchedulingPolicy::NON_MAIN_WORKER: {
            if (!JobWorkerThreadGroup::HasOnlyWorker(groupId, MAIN_WORKER_ID)) {
...
Coroutine *StackfulCoroutineManager::GetCoroutineInstanceForLaunch(Job *job, AffinityMask affinityMask)
{
    ...
    if (co != nullptr) {
        ReuseCoroutineInstance(co, job);
    } else {
        co = CreateCoroutineInstance(job);
    }
    if (co == nullptr) {
        LOG(DEBUG, COROUTINES)
    ...
    co->GetJob()->SetAffinityMask(affinityMask);
    return co;
}

LaunchResult StackfulCoroutineManager::LaunchImpl(Job *job, const JobWorkerThreadGroup::Id &groupId,
                                                  JobEvent *startEvent)
{
#ifndef NDEBUG
    ...
    co = GetCoroutineInstanceForLaunch(job, affinityMask);
    if (co == nullptr) {
    ...
    }
    {
        ...
        if (startEvent == nullptr) {
            ...
            w->AddCoroutineWaitingForStart(startEvent, co);
        }
    }
#ifndef NDEBUG
...
LaunchResult StackfulCoroutineManager::LaunchImmediatelyImpl(Job *job, const JobWorkerThreadGroup::Id &groupId)
{
    Coroutine *co = nullptr;
    ASSERT(JobWorkerThreadGroup::HasOnlyWorker(groupId, Coroutine::GetCurrent()->GetWorker()->GetId()));
    auto affinityMask = CalcAffinityMask(groupId);
    ...
    GetCurrentWorker()->GetPerfStats().FinishInterval(JobTimeStats::LAUNCH);
    ...
    GetCurrentWorker()->GetPerfStats().StartInterval(JobTimeStats::LAUNCH);

    return LaunchResult::OK;
}

static bool ValidateLaunchArgs([[maybe_unused]] Coroutine *co, JobWorkerThread *w, const LaunchParams &params)
{
    ASSERT(co != nullptr);
    ASSERT(w == co->GetContext<StackfulCoroutineContext>()->GetWorker());

    if UNLIKELY ((params.startEvent != nullptr) && params.launchImmediately) {
        LOG(FATAL, COROUTINES) << "Can't delay immediate launch";
        return false;
    }
    if UNLIKELY ((params.startEvent != nullptr) && (params.startEvent->GetType() == JobEvent::Type::TIMER) &&
                 (params.groupId != ark::JobWorkerThreadGroup::GenerateExactWorkerId(w->GetId()))) {
        LOG(FATAL, COROUTINES) << "delayed launch is supported on the same worker only";
        return false;
    }
...
LaunchResult StackfulCoroutineManager::Launch(Job *job, const LaunchParams &params)
{
    ...
    if (params.launchImmediately) {
        ...
        if (groupId == JobWorkerThreadGroup::AnyId() && w->InExclusiveMode()) {
...
void StackfulCoroutineManager::DumpJobStats() const
{
...
void StackfulCoroutineManager::MainCoroutineCompleted()
{
    ...
    LOG(DEBUG, COROUTINES) << "StackfulCoroutineManager::MainCoroutineCompleted(): stopping workers";
    {
        ...
        while (activeWorkersCount_ > 1) {  // 1 is for MAIN
            ...
            GetCurrentWorker()->GetPerfStats().FinishInterval(JobTimeStats::SCH_ALL);
            // NOTE(konstanting, #IAD5MH): need timed wait?..
            workersCv_.Wait(&workersLock_);
...
Coroutine *StackfulCoroutineManager::CreateNativeCoroutine(Runtime *runtime, PandaVM *vm,
                                                           Job::NativeEntrypointInfo::NativeEntrypointFunc entry,
                                                           ...
                                                           CoroutinePriority priority)
{
    ...
    auto *job = CreateJob(std::move(name), Job::NativeEntrypointInfo(entry, param), priority, type);
    ASSERT(job != nullptr);
...
void StackfulCoroutineManager::OnNativeCallExit(JobExecutionContext *executionCtx)
{
    if (GetConfig().enableDrainQueueIface) {
        ...
        ASSERT(executionCtx == Coroutine::GetCurrent());
...
JobExecutionContext *StackfulCoroutineManager::AttachExclusiveWorker(Runtime *runtime, PandaVM *vm)
{
    ...
    auto *eWorker = CreateWorker(runtime, vm, StackfulCoroutineWorker::ScheduleLoopType::FIBER, "[e-worker] ", true);
...
bool StackfulCoroutineManager::DetachExclusiveWorker()
{
...
bool StackfulCoroutineManager::MigrateCoroutinesInward(StackfulCoroutineWorker *to)
{
    ...
    auto affinityMask = CalcAffinityMask(JobWorkerThreadGroup::AnyId());
    bool res;
    {
        os::memory::LockHolder lkWorkers(workersLock_);
    ...
    if (res) {
        ...
        to->CacheLocalObjectsInExecutionCtx();
    }
    return res;
}
...
void StackfulCoroutineManager::InitializeManagedStructures(const CoroutineWorker::CreatePluginObjFunc &createEtsObj)
{
    ...
    for (size_t i = 0; i < numWorkers; i++) {
        eachWorkerLocalObjects.push_back(JobWorkerThread::CreateWorkerLocalObjects(createEtsObjFunc_));
    }
    workersLock_.Lock();
