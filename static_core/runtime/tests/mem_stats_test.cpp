...
class RawStatsBeforeTest {
    size_t raw_bytes_allocated_before_test_;
    size_t raw_bytes_freed_before_test_;
    size_t raw_bytes_footprint_before_rest_;

public:
    explicit RawStatsBeforeTest(MemStatsType *stats)
        : raw_bytes_allocated_before_test_(stats->GetAllocated(SpaceType::SPACE_TYPE_INTERNAL)),
          raw_bytes_freed_before_test_(stats->GetFreed(SpaceType::SPACE_TYPE_INTERNAL)),
          raw_bytes_footprint_before_rest_(stats->GetFootprint(SpaceType::SPACE_TYPE_INTERNAL))
    {
    }

    [[nodiscard]] size_t GetRawBytesAllocatedBeforeTest() const
    {
        return raw_bytes_allocated_before_test_;
    }

    [[nodiscard]] size_t GetRawBytesFreedBeforeTest() const
    {
        return raw_bytes_freed_before_test_;
    }
...
void AssertHeapStats(MemStatsType *stats, size_t bytes_in_heap, size_t heap_bytes_allocated, size_t heap_bytes_freed)
{
    ASSERT_EQ(heap_bytes_allocated, stats->GetAllocated(SpaceType::SPACE_TYPE_OBJECT));
    ASSERT_EQ(heap_bytes_freed, stats->GetFreed(SpaceType::SPACE_TYPE_OBJECT));
    ASSERT_EQ(bytes_in_heap, stats->GetFootprint(SpaceType::SPACE_TYPE_OBJECT));
}

void AssertHeapHumongousStats(MemStatsType *stats, size_t bytes_in_heap, size_t heap_bytes_allocated,
                              size_t heap_bytes_freed)
{
    ASSERT_EQ(heap_bytes_allocated, stats->GetAllocated(SpaceType::SPACE_TYPE_HUMONGOUS_OBJECT));
    ASSERT_EQ(heap_bytes_freed, stats->GetFreed(SpaceType::SPACE_TYPE_HUMONGOUS_OBJECT));
    ASSERT_EQ(bytes_in_heap, stats->GetFootprint(SpaceType::SPACE_TYPE_HUMONGOUS_OBJECT));
}

void AssertHeapObjectsStats(MemStatsType *stats, size_t heap_objects_allocated, size_t heap_objects_freed,
                            size_t heap_humungous_objects_allocated, size_t heap_humungous_objects_freed)
{
    ASSERT_EQ(heap_objects_allocated, stats->GetTotalObjectsAllocated());
    ASSERT_EQ(heap_objects_freed, stats->GetTotalObjectsFreed());

    // On arm-32 platform, we should cast the uint64_t(-1) to size_t(-1)
    ASSERT_EQ(heap_objects_allocated - heap_humungous_objects_allocated,
              static_cast<size_t>(stats->GetTotalRegularObjectsAllocated()));
    ASSERT_EQ(heap_objects_freed - heap_humungous_objects_freed,
              static_cast<size_t>(stats->GetTotalRegularObjectsFreed()));

    ASSERT_EQ(heap_humungous_objects_allocated, stats->GetTotalHumongousObjectsAllocated());
    ASSERT_EQ(heap_humungous_objects_freed, stats->GetTotalHumongousObjectsFreed());

    ASSERT_EQ(heap_objects_allocated - heap_objects_freed, stats->GetObjectsCountAlive());
    ASSERT_EQ(heap_objects_allocated - heap_objects_freed + heap_humungous_objects_allocated -
                  heap_humungous_objects_freed,
              stats->GetRegularObjectsCountAlive());
    ASSERT_EQ(heap_humungous_objects_allocated - heap_humungous_objects_freed, stats->GetHumonguousObjectsCountAlive());
}
...
void AssertRawStats(MemStatsType *stats, size_t raw_bytes_allocated, size_t raw_bytes_freed, size_t raw_bytes_footprint,
                    RawStatsBeforeTest &stats_before_test)
{
    ASSERT_EQ(raw_bytes_allocated + stats_before_test.GetRawBytesAllocatedBeforeTest(),
              stats->GetAllocated(SpaceType::SPACE_TYPE_INTERNAL));
    ASSERT_EQ(raw_bytes_freed + stats_before_test.GetRawBytesFreedBeforeTest(),
              stats->GetFreed(SpaceType::SPACE_TYPE_INTERNAL));
    ASSERT_EQ(raw_bytes_footprint + stats_before_test.GetRawBytesFootprintBeforeTest(),
