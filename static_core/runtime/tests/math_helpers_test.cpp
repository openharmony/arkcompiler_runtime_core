...
void TestBitShl()
{
    ...
    {
        T v = 1;
        size_t shift = std::numeric_limits<UnsignedType>::digits - 1;
        T res = bit_shl<T>()(v, shift);
    ...
    {
        T v = 1;
        size_t shift = std::numeric_limits<UnsignedType>::digits;
        T res = bit_shl<T>()(v, shift);
...
void TestBitShr()
{
...

    {
        ...
        T shift = std::numeric_limits<UnsignedType>::digits - 1;
        T res = bit_shr<T>()(v, shift);
    ...
    {
        T v = 1;
        T shift = std::numeric_limits<UnsignedType>::digits;
        T res = bit_shr<T>()(v, shift);
        EXPECT_EQ(res, v) << ss.str();
    }
