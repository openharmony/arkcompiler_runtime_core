...
public:
    ...
    struct IntWrapper {
        IntWrapper() = default;
        ~IntWrapper() = default;
        explicit IntWrapper(int new_element) : element_(new_element) {}
        IntWrapper(const IntWrapper &new_wrapper) = default;
        IntWrapper &operator=(const IntWrapper &new_wrapper) = default;
        bool operator<(const IntWrapper &other_wrapper) const
        {
            return element_ < other_wrapper.element_;
        }
        bool operator==(const IntWrapper &other_wrapper) const
        {
            return element_ == other_wrapper.element_;
        ...
        IntWrapper operator+(const IntWrapper &other_wrapper) const
        {
            return IntWrapper(element_ + other_wrapper.element_);
        }
        void operator+=(const IntWrapper &other_wrapper)
        {
            element_ += other_wrapper.element_;
        }
        IntWrapper operator*(const IntWrapper &other_wrapper) const
        {
            return IntWrapper(element_ * other_wrapper.element_);
