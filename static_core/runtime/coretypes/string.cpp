                                      ...
                                      PandaVM *vm, bool movable, bool pinned)
{
    ...
    ark::mem::BaseString *string =
        movable ? reinterpret_cast<ark::mem::BaseString *>(
                      ...
                      vm->GetHeapManager()->AllocateObject(stringClass, size, DEFAULT_ALIGNMENT, thread,
                                                           ...
                                                           pinned))
                : reinterpret_cast<ark::mem::BaseString *>(vm->GetHeapManager()->AllocateNonMovableObject(
                      // CC-OFFNXT(G.FMT.06-CPP) project code style
...
String *String::FastSubUtf8String(String *src, uint32_t start, uint32_t length, const LanguageContext &ctx)
{
    ...
    // 3. copy it
    ark::common_vm::Span<uint8_t> dest(lineStrHandle->GetDataUtf8Writable(), length);
    // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
    ark::common_vm::Span<const uint8_t> source(srcFlat.GetDataUtf8() + start, length);
    ark::mem::BaseString::MemCopyChars(dest, length, source, length);
...
String *String::FastSubUtf16String(String *src, uint32_t start, uint32_t length, const LanguageContext &ctx)
{
    ...
    FlatStringInfo srcFlat = FlatStringInfo::FlattenAllString(srcHandle, ctx);
    // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
    bool canBeCompressed = ark::mem::BaseString::CanBeCompressed(srcFlat.GetDataUtf16() + start, length);
    ...
    if (canBeCompressed) {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        ark::mem::BaseString::CopyChars(lineStrHandle->GetDataUtf8Writable(), srcFlat.GetDataUtf16() + start, length);
    } else {
        uint32_t len = length * (sizeof(uint16_t) / sizeof(uint8_t));
        ark::common_vm::Span<uint16_t> dest(lineStrHandle->GetDataUtf16Writable(), length);
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        ark::common_vm::Span<const uint16_t> source(srcFlat.GetDataUtf16() + start, length);
        ark::mem::BaseString::MemCopyChars(dest, len, source, len);
    }
    return lineStrHandle.GetPtr();
}
...
String *String::FastSubString(String *src, uint32_t start, uint32_t length, const LanguageContext &ctx, PandaVM *vm)
{
    ...
    if (length < ark::mem::SlicedString::MIN_SLICED_STRING_LENGTH) {
        if (src->IsUtf8()) {
    ...
    if (src->IsUtf16()) {
        ...
        if (canBeCompressed) {
            ...
            srcFlat = FlatStringInfo::FlattenAllString(srcHandle, ctx);
            // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
            ark::mem::BaseString::CopyChars(lineStrHandle->GetDataUtf8Writable(), srcFlat.GetDataUtf16() + start,
                                            length);
            return lineStrHandle.GetPtr();
        }
    }
                                           ...
                                           const LanguageContext &ctx, PandaVM *vm)
{
    ...
    // copy left part
    ark::common_vm::Span<uint8_t> sp(newStringHandle->GetDataUtf8Writable(), newLength);
    ark::common_vm::Span<const uint8_t> src1(
        str1Handle->IsTreeString() ? str1Handle->GetTreeStringDataUtf8(tree8Buf1) : str1Handle->GetDataUtf8(), length1);
    ark::mem::BaseString::MemCopyChars(sp, newLength, src1, length1);

    // copy right part
    sp = sp.SubSpan(length1);
    ark::common_vm::Span<const uint8_t> src2(
        str2Handle->IsTreeString() ? str2Handle->GetTreeStringDataUtf8(tree8Buf2) : str2Handle->GetDataUtf8(), length2);
    ark::mem::BaseString::MemCopyChars(sp, length2, src2, length2);
    TSAN_ANNOTATE_IGNORE_WRITES_END();
    // String is supposed to be a constant object, so all its data should be visible by all threads
    arch::FullMemoryBarrier();
                                             ...
                                             const LanguageContext &ctx, PandaVM *vm)
{
    ...
    ark::common_vm::Span<uint16_t> sp(newStringHandle->GetDataUtf16Writable(), newLength);
    if (str1Handle->IsUtf8()) {
        ark::mem::BaseString::CopyChars(sp.data(),
                                        str1Handle->IsTreeString() ? str1Handle->GetTreeStringDataUtf8(tree8Buf1)
                                                                   : str1Handle->GetDataUtf8(),
                                        length1);
    } else {
        ark::common_vm::Span<const uint16_t> src1(
            str1Handle->IsTreeString() ? str1Handle->GetTreeStringDataUtf16(tree16Buf1) : str1Handle->GetDataUtf16(),
            length1);
        ark::mem::BaseString::MemCopyChars(sp, newLength << 1U, src1, length1 << 1U);
    }
    // copy right part
    PandaVector<uint8_t> tree8Buf2;
    ...
    if (str2Handle->IsUtf8()) {
        ...
        ark::common_vm::Span<const uint16_t> src2(
            str2Handle->IsTreeString() ? str2Handle->GetTreeStringDataUtf16(tree16Buf2) : str2Handle->GetDataUtf16(),
            length2);
        ark::mem::BaseString::MemCopyChars(sp, length2 << 1U, src2, length2 << 1U);
    }
...
String *String::Concat(String *str1, String *str2, const LanguageContext &ctx, PandaVM *vm)
{
    ...
    if (newLength >= ark::mem::TreeString::MIN_TREE_STRING_LENGTH) {
                                ...
                                bool movable, bool pinned)
{
    ...
    FlatStringInfo srcFlat = FlatStringInfo::FlattenAllString(srcHandle, ctx);
    auto slicedStr = ark::mem::SlicedString::Cast(baseStrHandle->ToString());
    slicedStr->InitLengthAndFlags(length, srcFlat.GetString()->IsMUtf8());
    slicedStr->SetMixHashcode(0);
    auto writeBarrier = [](void *obj, size_t offset, ark::common_vm::BaseObject *str) {
...
String *String::AllocSlicedStringObject(const LanguageContext &ctx, PandaVM *vm, bool movable, bool pinned)
{
    ...
    size_t size = ark::mem::SlicedString::SIZE;
    ark::mem::BaseString *slicedStr = nullptr;
    if (movable) {
        slicedStr = reinterpret_cast<ark::mem::BaseString *>(
        ...
        slicedStr = reinterpret_cast<ark::mem::BaseString *>(vm->GetHeapManager()->AllocateNonMovableObject(
                                      ...
                                      bool pinned)
{
    ...
    size_t size = ark::mem::TreeString::SIZE;
    ark::mem::BaseString *treeStr = nullptr;
    if (movable) {
        treeStr = reinterpret_cast<ark::mem::BaseString *>(
            vm->GetHeapManager()->AllocateObject(treeStrCls, size, DEFAULT_ALIGNMENT, thread,
                                                 mem::ObjectAllocatorBase::ObjMemInitPolicy::REQUIRE_INIT, pinned));
    } else {
        treeStr = reinterpret_cast<ark::mem::BaseString *>(vm->GetHeapManager()->AllocateNonMovableObject(
                                 ...
                                 bool movable, bool pinned)
{
    ...
    auto *treeStr = ark::mem::TreeString::Cast(baseStr->ToString());
    ...
    auto writeBarrierLeft = [thread](void *obj, size_t offset, ark::common_vm::BaseObject *str) {
    ...
    auto writeBarrierRight = [thread](void *obj, size_t offset, ark::common_vm::BaseObject *str) {
        ObjectAccessor::SetObject(thread, obj, offset, reinterpret_cast<ObjectHeader *>(str));
    };
...
int32_t String::Compare(VMHandle<String> &left, VMHandle<String> &right, const LanguageContext &ctx)
{
    ...
    static constexpr bool withNativeMemory = true;
    FlatStringInfo lflat = FlatStringInfo::FlattenAllString(left, ctx, withNativeMemory);
    FlatStringInfo rflat = FlatStringInfo::FlattenAllString(right, ctx, withNativeMemory);
    ...
    if (lflat.IsUtf8()) {
        ...
        if (right.GetPtr()->IsMUtf8()) {
            ark::common_vm::Span<const uint8_t> lspan(lflat.GetDataUtf8(), lCount);
            ark::common_vm::Span<const uint8_t> rspan(rflat.GetDataUtf8(), rCount);
            int32_t charDiff = ark::mem::CompareStringSpan(lspan, rspan, minCount);
            if (charDiff != 0) {
                return charDiff;
            }
            ...
            ark::common_vm::Span<const uint8_t> lspan(lflat.GetDataUtf8(), lCount);
            ark::common_vm::Span<const uint16_t> rspan(rflat.GetDataUtf16(), rCount);
            int32_t charDiff = ark::mem::CompareStringSpan(lspan, rspan, minCount);
            if (charDiff != 0) {
                return charDiff;
            }
        ...
        if (right.GetPtr()->IsUtf16()) {
            ark::common_vm::Span<const uint16_t> lspan(lflat.GetDataUtf16(), lCount);
            ark::common_vm::Span<const uint16_t> rspan(rflat.GetDataUtf16(), rCount);
            int32_t charDiff = ark::mem::CompareStringSpan(lspan, rspan, minCount);
            if (charDiff != 0) {
                return charDiff;
...
bool String::CanBeCompressedUtf16(const uint16_t *utf16Data, uint32_t utf16Length, uint16_t non)
{
    ...
    for (uint32_t i = 0; i < utf16Length; i++) {
        if (!ark::mem::BaseString::IsASCIICharacter(data[i]) && data[i] != non) {
            isCompressed = false;
            break;
        }
    }
    ...
    if (str1->GetLength() != utf16Length) {
        ...
        if (str1CanBeCompressed) {
            ...
            const uint8_t *str1Utf8DataFlat =
                ark::mem::BaseString::GetUtf8DataFlat(readBarrier, str1->ToString(), bufStr1Utf8);
            ark::common_vm::Span<const uint8_t> data1(str1Utf8DataFlat, str1Count);
            ark::common_vm::Span<const uint8_t> data2(mutf8Data, utf16Length);
            result = ark::mem::BaseString::StringsAreEquals(data1, data2);
        } else {
            std::vector<uint16_t> bufStr1Utf16;
            const uint16_t *str1Utf16DataFlat =
                ark::mem::BaseString::GetUtf16DataFlat(readBarrier, str1->ToString(), bufStr1Utf16);
...
bool String::StringsAreEqual(String *str1, String *str2)
{
    ...
    return ark::mem::BaseString::StringsAreEqual(std::move(readBarrier), str1->ToString(), str2->ToString());
}
