...
class Instruction < SimpleDelegator
  ...
  def format
    Panda.format_hash[dig(:format)] || Quick.format_hash[dig(:format)]
  end
...
class Format
  ...
  cached def encoding
    ...
    name.sub('pref_', '').sub('op_', '').sub('none_', '').split('_').each_slice(2).map do |name, width|
      op = OpenStruct.new
      op.name = name
      op.width = width.to_i
      op.offset = offset
  ...
  # Return [offset, width] for profile id element
  def profile_info
    raise "Format #{name} has no profile" unless profiled?

    prof = encoding.values.select { |x| x.name == 'prof' }
    raise "Profile info not found in format #{name}" if prof.size != 1

    [prof[0].offset, prof[0].width]
  end
...
class Operand
  ...
  def initialize(name, srcdst, type, width = 0, offset = 0)
    ...
    unless %i[v acc imm method_id type_id field_id string_id literalarray_id prof].include?(@name)
...
module Panda
  ...
  def verify_schema(name, data, schema)
    data.each do |item|
      item.each_pair do |key, value|
        element = schema[key]
        raise "Schema verification failed for #{name}: no element in schema: #{key}" unless element

        case element
        when 'string'
          raise "Schema verification failed for #{name}: #{key} must be a string" unless value.is_a? String
        when 'int'
          raise "Schema verification failed for #{name}: #{key} must be an integer" unless value.is_a? Integer
        when Array
          value.each do |x|
            raise "Schema verification failed for #{name}: unexpected array value: #{x}" unless element.include? x
          end
        end
      end
    end
  ...
  cached def profiles
    verify_schema('Profiles', @data.profiles, @data.profiles_schema)
    @data.profiles.map { |x| [x.name, x] }.to_h
  end

  def quickened_plugins
    @data.namespaces.map { |i| [i.namespace, i.used_instructions] if i.quickening }.compact.to_h
  end
  ...
  cached def instructions
    ...
    tmp_public = initialize_instructions(opcodes, &:opcode_idx)
    tmp_private = initialize_instructions(opcodes) { |ins| !ins.opcode_idx }
  ...
  cached def prefixes
    ...
    tmp_public = initialize_prefixes(opcodes, &:opcode_idx)
    tmp_private = initialize_prefixes(opcodes) { |p| !p.opcode_idx }
    tmp = tmp_public + tmp_private
  ...
  def formats
    format_hash.merge(Quick.format_hash).values.uniq(&:pretty).sort_by(&:pretty)
  end
  ...
  cached def each_data_instruction
...
module Quick
  ...
  def init
    @format_hash = {}
    @select = Hash.new { |h, k| h[k] = [] }
    Panda.each_data_instruction.each do |insn|
      add_to_quick(OpenStruct.new(insn)) if !insn.namespace || Panda.quickened_plugins[insn.namespace]
    end
  end

  def add_to_quick(insn)
    if insn.namespace
      insn.format = remove_pref(insn.format)
      insn.prefix = ''
    end
    ins = Instruction.new(insn)
    @format_hash[insn.format] = Format.new(insn.format)
    if ins.namespace == 'core'
      Panda.quickened_plugins.each do |ns, used|
        @select[ns].push(ins.clone) if used.include?(ins.mnemonic)
      end
    else
      raise "Plugin #{ins.namespace} is not quickened" unless Panda.quickened_plugins[insn.namespace]

      @select[ins.namespace].push(ins)
    end
  end
  ...
  def remove_pref(str)
    str.sub('_PREF_NONE', '').sub('_pref_none', '').sub('PREF_', '').sub('pref_', '')
  end
end
